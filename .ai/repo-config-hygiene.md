...
This monorepo is polyglot: 6 TypeScript/JavaScript apps, 1 Python app (`analytics-engine`), plus shared TS `packages/*`. The two scope tiers below reflect what each rule can verify.

### Tier A — All 7 apps (language-agnostic contract)

Apps in scope: `account-engine`, `alpha-etl`, `app`, `desktop`, `landing-page`, `podcast-pipeline`, `analytics-engine`.

Used by these rules only:

- `3.5 missing_standard_script` — CLAUDE.md states _every_ app exposes the same `pnpm <script>` surface regardless of underlying language. analytics-engine wraps `uv run`, mobile wraps `dart`/`flutter`, but the CLI is uniform.
...
In scope: `apps/{account-engine,alpha-etl,app,desktop,landing-page,podcast-pipeline}` and all `packages/*`.
...
- `pyproject.toml` cross-app comparisons — there is currently only one Python app, so same-language drift has no peers to compare against. Do not invent findings.
...
2. Treat every option inherited via `extends` as the _intended_ baseline.
...
Same dependency name, different _non-`catalog:`_ versions across two or more in-scope apps/packages.
...
- `"declaration": false` / `"declarationMap": false` in application tsconfigs (these are intentional non-library overrides).
...
- Tier B rules (`version_mismatch`, `unpinned_catalog_candidate`, `tsconfig_drift`, `eslint_inconsistency`, cross-app-import half of `structure_issue`) skip `apps/analytics-engine` — it has no `tsconfig.json` and no npm-tracked runtime deps.
...
- There is currently exactly one Python app, so same-language config drift has no peers to compare. Do NOT invent `python_config_drift` findings.
