...
- Identity (zapEngine): All repository operations MUST use GitHub account `i-xtsu-sixyou-ken-mei`; never use `david30907d`. Before local `git commit`/`git push`/`gh pr create`, verify `git config user.name` is `i-xtsu-sixyou-ken-mei`, `git config user.email` is `i-xtsu-sixyou-ken-mei@users.noreply.github.com`, and `gh auth status` shows `i-xtsu-sixyou-ken-mei` as the active account. If any local setting mismatches, run `gh auth switch --user i-xtsu-sixyou-ken-mei` and `git config user.name "i-xtsu-sixyou-ken-mei" && git config user.email "i-xtsu-sixyou-ken-mei@users.noreply.github.com"`. For commits created through the authenticated GitHub App/API rather than local git, require the authenticated account to be `i-xtsu-sixyou-ken-mei`; GitHub may record that account's verified relay email instead of the local noreply address.
