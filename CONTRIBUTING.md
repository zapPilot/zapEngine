...
The production job uses the `production` GitHub Environment, reviews the exact pending set with `supabase db push --dry-run`, applies it with `supabase db push`, and proves a second dry-run is empty before reporting `supabase migration list`. Production deploys are serialized and queued without cancelling older runs. Configure `SUPABASE_DB_PASSWORD` and `SUPABASE_PROJECT_ID` as Environment or repository secrets; the job pushes through the session pooler with the database password, so it needs no Supabase access token.
...
Every new table in `alpha_raw` or `ops` must enable RLS in the migration that creates it. These schemas are outside the Data API, so the Supabase security advisor never reports a missing RLS there; the `Supabase migrations` PR workflow checks it instead. An `alpha_raw` table gets the `direct_etl_full_access` policy (`for all to alpha_etl_user`) and the `direct_readonly_select` policy (`for select to readonly_user`). An `ops` table either stays deny-all behind SECURITY DEFINER RPCs or pairs a `service_role` grant with a `service_role` policy. A new role that connects to Postgres directly without BYPASSRLS must be added to the policies of every table it uses: a role that no policy names reads zero rows and updates or deletes nothing, without raising an error. The same workflow fails when a direct-connect role holds a grant on an `alpha_raw` or `ops` table that no policy covers. See `supabase/migrations/20260924150000_close_internal_schema_rls_gaps.sql`.
