...
-- Date: 2026-07-07
-- Part of: event-sourced strategy plane (append-only ledger event tables)
--
...
-- are projections. analytics-engine computes and stays
...
-- REVOKE (grants) plus a guard trigger (defence in depth).
--
...
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'ledger tables are append-only: % on % is forbidden',
    TG_OP, TG_TABLE_NAME;
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON TABLE ledger_signal_events IS
  'Append-only: strategy inputs (regime state, daily suggestion) as observed. Source of truth for the decision chain.';
COMMENT ON TABLE ledger_decision_events IS
  'Append-only: strategy decisions. strategy_version + config_identity close the implicit-versioning gap.';
COMMENT ON TABLE ledger_plan_events IS
  'Append-only: composed execution plans. plan_hash is nullable until plan-integrity primitives land.';
COMMENT ON TABLE ledger_execution_events IS
  'Append-only: per-leg execution outcomes. Doubles as the L3 journal and the resumable leg-state store.';
