...
In-memory async work queue and processors for weekly reports and daily suggestions.
...
4. Batch jobs fan out into single-user child jobs; `GET /jobs/:jobId` derives parent status from the children.
