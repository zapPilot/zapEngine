...
function printDuplicateCheckStep(duplicateStats: DuplicateStats): void {
  ...
  if (duplicateStats.duplicateCount > 0) {
    const duplicatePercent = ((duplicateStats.duplicateCount / duplicateStats.totalRows) * 100).toFixed(1);
...
function printUserStatsRow(stats: UserStats): void {
  ...
  console.log(`   ⚠️  Issue detected! Should be ${stats.expectedRows} but got ${stats.actualRows}`);
  if (stats.actualRows === cartesianProduct) {
    console.log(`   ⚠️  Cartesian product detected: ${stats.subscriptionCount} × ${stats.walletCount} = ${cartesianProduct}`);
  }
  console.log('');
}
...
function printCartesianProductStep(userStats: UserStats[]): number {
  ...
  console.log(`Found ${userStats.length} users with multiple subscriptions or wallets:\n`);
  let issueCount = 0;
...
function printFinalVerdict(allPassed: boolean): void {
  console.log(`\n${DIVIDER}`);
  if (allPassed) {
    console.log('✅ VALIDATION PASSED: No duplicates or Cartesian product issues');
    console.log(`${DIVIDER}\n`);
    return;
  }

  console.log('❌ VALIDATION FAILED: Duplicates or Cartesian product issues found');
  console.log(`${DIVIDER}\n`);
}
...
async function analyzeCartesianProduct(): Promise<UserStats[]> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const hasIssue = actualRowCount !== expectedRows || actualRowCount === cartesianProduct;
...
async function validate(): Promise<void> {
  ...
  try {
    ...
    // Final verdict
    const allPassed = duplicateStats.duplicateCount === 0 && userStats.every((stats) => !stats.hasIssue);
...
export { validate, checkDuplicates, analyzeCartesianProduct };
