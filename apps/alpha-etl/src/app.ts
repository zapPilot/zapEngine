...
import { closeDbPool, testDatabaseConnection } from './config/database.js';
...
if (shouldForceListenHost) {
  ...
  app.listen = ((
    ...args: Parameters<typeof app.listen>
...
async function runInitialDatabaseCheck(): Promise<void> {
  try {
    ...
    logger.error(
      'Failed to complete initial database connectivity check:',
      error,
    );
    process.exit(1);
  }
