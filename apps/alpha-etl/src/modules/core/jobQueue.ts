...
import { type MVRefreshStats, mvRefresher } from '../../modules/core/mvRefresh.js';
...
type PipelineProcessResult = Awaited<ReturnType<ETLPipelineFactory['processJob']>>;
...
export class ETLJobQueue {
  ...
  async enqueue(params: Pick<ETLJob, 'trigger' | 'sources' | 'filters' | 'metadata'>): Promise<ETLJob> {
  ...
  private async persistJobStatus(
    ...
    try {
      ...
      const { query, params } = buildPersistJobStatusQuery(jobId, status, metadata);
  ...
  private async executeJob(job: ETLJob): Promise<ProcessedJobOutcome> {
    logger.info('Starting ETL job processing', { jobId: job.jobId, sources: job.sources });

    const startTime = Date.now();
    const pipelineResult = await this.processor.processJob(job);
    const etlDurationMs = Date.now() - startTime;
    const mvRefreshStats = await this.refreshMaterializedViewsIfNeeded(job, pipelineResult, startTime);
  ...
  private async refreshMaterializedViewsIfNeeded(
    ...
    if (!pipelineResult.success && pipelineResult.recordsInserted > 0) {
      logger.warn('Refreshing MVs despite ETL partial failure because records were inserted', {
        jobId: job.jobId,
        recordsInserted: pipelineResult.recordsInserted,
  ...
  private async finalizeProcessedJob(
    ...
    const finalStatus: 'completed' | 'failed' = outcome.jobSuccess ? 'completed' : 'failed';
    ...
    await this.persistJobStatus(job.jobId, job.status, {
      ...job.metadata,
      errorMessage: getPersistedErrorMessage(job.metadata, outcome.jobSuccess, outcome.mvRefreshStats)
    });
  }
  ...
  getQueueStatus(): QueueStatus {
    ...
    const counts: Pick<QueueStatus, 'pending' | 'processing' | 'completed' | 'failed'> = {
      pending: 0,
      processing: 0,
      completed: 0,
      failed: 0
