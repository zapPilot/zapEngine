...
export class DeFiLlamaFetcher extends BaseApiFetcher {
  ...
  async fetchAllPools(tvlThreshold = 0): Promise<PoolData[]> {
    try {
      ...
      if (data.status !== 'success') {
        throw new Error(`DeFiLlama API returned non-success status: ${data.status}`);
      }
  ...
  async fetchPoolsByChain(chain: string, tvlThreshold = 0): Promise<PoolData[]> {
    const allPools = await this.fetchAllPools(tvlThreshold);
    const mappedChain = mapChainName(chain);
    return allPools.filter(pool =>
      pool.chain.toLowerCase() === mappedChain.toLowerCase()
    );
  }
  async findMatchingPool(
    ...
    try {
      ...
      const matchedPool = this.findFirstMatchingPool(pools, mappedChain, projectId, version, normalizedSymbols);
      if (matchedPool) {
  ...
  private matchesProject(pool: PoolData, projectId: string, version: string): boolean {
    ...
    const doesVersionMatch = version === '0' || poolProject.includes(normalizedVersion);
  ...
  private matchesSymbols(pool: PoolData, targetSymbols: string[]): boolean {
    ...
    return checkSymbolListsEqual(poolSymbols, targetSymbols, true)
  ...
  private parseDeFiLlamaResponse(rawData: unknown, url: string): DeFiLlamaResponse {
  ...
  private filterPoolsByTvl(pools: DeFiLlamaPool[], tvlThreshold: number): DeFiLlamaPool[] {
