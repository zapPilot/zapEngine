...
export type { CoinMarketCapFearGreedResponse, SentimentData } from '../../modules/sentiment/schema.js';
...
export class FearGreedFetcher extends BaseApiFetcher {
  ...
  private static readonly RATE_LIMIT_MS = process.env.NODE_ENV === 'test'
    ? 0
  ...
  constructor(config?: FearGreedConfig) {
    const baseUrl = config?.apiUrl ?? process.env.COINMARKETCAP_API_URL ?? FearGreedFetcher.BASE_URL;
    super(baseUrl, FearGreedFetcher.RATE_LIMIT_MS);

    this.apiKey = config?.apiKey ?? process.env.COINMARKETCAP_API_KEY ?? '';

    if (!this.apiKey) {
      logger.warn('FearGreedFetcher initialized without API key - requests will fail');
    } else {
  ...
  async fetchCurrentSentiment(): Promise<SentimentData> {
    try {
      ...
      const response = await this.fetchWithRetry<CoinMarketCapFearGreedResponse>(
        url,
      ...
      const normalizedData = normalizeSentimentData(sentimentEntry, FearGreedFetcher.SOURCE_NAME);
  ...
  private handleFetchCurrentSentimentError(error: unknown): never {
    ...
    logger.error('Failed to fetch Fear & Greed Index from CoinMarketCap', serializeError(error));
    throw error;
  }
}
