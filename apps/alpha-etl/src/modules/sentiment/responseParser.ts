...
function extractDataEntry(response: CoinMarketCapFearGreedResponse): CoinMarketCapFearGreedResponse['data'] {
...
function validateRequiredSentimentFields(entry: CoinMarketCapFearGreedResponse['data']): void {
  if (
    ...
    throw new Error('Invalid API response: missing required fields (value, value_classification, or update_time)');
  }
}
