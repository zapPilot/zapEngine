...
import { SentimentDataSchema, type SentimentData } from '../../modules/sentiment/schema.js';
...
export class SentimentDataTransformer {
  ...
  transform(rawData: SentimentData): SentimentSnapshotInsert | null {
    try {
      ...
      if (!this.isValidClassificationForValue(validated.value, validated.classification)) {
      ...
      const transformed = this.createSnapshotInsert(validated, rawData, snapshotTime);
  ...
  transformBatch(rawDataArray: SentimentData[]): SentimentSnapshotInsert[] {
    return transformBatchWithLogging(rawDataArray, (item) => this.transform(item), 'Sentiment data');
  }
  ...
  private isValidClassificationForValue(value: number, classification: string): boolean {
  ...
  private resolveClassificationRange(classification: string): [number, number] {
    return SENTIMENT_CLASSIFICATION_BOUNDARIES[classification]
      || [DATA_LIMITS.SENTIMENT_MIN, DATA_LIMITS.SENTIMENT_MAX];
  }

  private isTimestampOutsideExpectedRange(timestampMs: number, nowMs: number): boolean {
