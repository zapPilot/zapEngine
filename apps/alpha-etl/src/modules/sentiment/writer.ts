...
export class SentimentWriter extends BaseWriter<SentimentSnapshotInsert> {
  async writeSentimentSnapshots(
    ...
    logger.debug('Starting sentiment snapshots write', { source, snapshotCount: snapshots.length });
  ...
  private async writeBatch(
    ...
    const batchResult = await this.executeBatchWrite({
      ...
      buildQuery: () => {
        const { columns, placeholders, values } = buildSentimentInsertValues(validRecords);
        const query = `
