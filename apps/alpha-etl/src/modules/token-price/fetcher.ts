...
export class CoinGeckoFetcher extends BaseApiFetcher {
  ...
  constructor(config?: CoinGeckoFetcherConfig) {
    const baseUrl = config?.baseUrl ?? process.env.COINGECKO_API_URL ?? CoinGeckoFetcher.DEFAULT_BASE_URL;
    /* v8 ignore start -- production rate limit path not reachable in test env */
    const defaultRateLimit = process.env.NODE_ENV === 'test' ? 0 : (RATE_LIMITS.COINGECKO_DELAY_MS || 2000);
    /* v8 ignore stop */
  ...
  async fetchCurrentPrice(
    ...
    try {
      ...
      const response = await this.fetchCoinGecko<CoinGeckoSimplePriceResponse>(endpoint);
      const priceData = this.parseCurrentPriceResponse(response, tokenId, tokenSymbol);
  ...
  async fetchHistoricalPrice(
    ...
    try {
      ...
      const response = await this.fetchCoinGecko<CoinGeckoHistoricalResponse>(endpoint);
      const priceData = this.parseHistoricalPriceResponse(response, tokenId, tokenSymbol, date);
  ...
  private parseCurrentPriceResponse(
    ...
    if (!tokenData?.usd) {
      throw new Error(`Invalid CoinGecko response: missing ${tokenId}.usd field`);
    }
  ...
  private parseHistoricalPriceResponse(
    ...
    if (!parsed.success) {
      throw new Error(`Invalid CoinGecko historical response for ${tokenId} on ${date}: missing market_data.current_price.usd`);
    }
  ...
  private parseDate(ddMmYyyy: string): Date {
    ...
    return new Date(Date.UTC(parseInt(year, 10), parseInt(month, 10) - 1, parseInt(day, 10), 0, 0, 0, 0));
  }
  ...
  private handleFetchError(
    ...
    if (error instanceof APIError) {
      logger.error(
        isHistorical ? 'CoinGecko historical API request failed' : 'CoinGecko API request failed',
        { error: error.message, statusCode: error.statusCode, ...logContext }
      );
      const message = isHistorical && context.date
    ...
    logger.error(
      isHistorical ? 'Failed to fetch historical token price' : 'Failed to fetch current token price',
      { error, ...logContext }
    );
    throw error;
  }
