...
- Per-wallet emptiness stays a success (see above), but a scheduled batch where every fetched wallet returned no tokens and no positions fails the source: DeBank answering 200 with nothing is not a day on which every wallet emptied out. Keep the assertion at batch level so the writers still clear each wallet/day slice.
- Which wallets the scheduled batch refreshes is decided by `public.get_user_service_states()`, not here, and it is decided per provider: `due_sources` names the providers that have not landed data for that wallet inside the 20-hour fence. `modules/user-service/selector.ts` only projects that answer; re-deriving due-ness in TypeScript makes the Control Center and this job disagree about who is Priority, and one wallet-level timestamp cannot say that DeBank landed today while Hyperliquid did not.
- Freshness is claimed only once the load has committed. A failed batch, or the silently empty one above, records every fetched wallet as failed in `ops.wallet_source_refresh_state` and leaves `user_crypto_wallets.last_portfolio_update_at` unstamped, so those wallets stay due. Over-refreshing costs one extra DeBank call; calling a wallet fresh on data that never landed loses the day.
