...
- Which wallets the scheduled batch refreshes is decided by `public.get_user_service_states()`, not here. `modules/user-service/selector.ts` only projects that answer; re-deriving due-ness in TypeScript makes the Control Center and this job disagree about who is Priority.
