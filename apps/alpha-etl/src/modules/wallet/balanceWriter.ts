...
import { WALLET_BALANCE_COLUMNS, buildInsertValues } from '../../core/database/columnDefinitions.js';

export class WalletBalanceWriter extends BaseWriter<WalletBalanceSnapshotInsert> {
  async writeWalletBalanceSnapshots(snapshots: WalletBalanceSnapshotInsert[]): Promise<WriteResult> {
  ...
  private async writeBatch(batch: WalletBalanceSnapshotInsert[], batchNumber: number): Promise<WriteResult> {
    return this.executeBatchWrite({
      batchNumber,
      logContext: 'wallet balance',
      recordCount: batch.length,
      buildQuery: () => {
        const { columns, placeholders, values } = buildInsertValues(batch, WALLET_BALANCE_COLUMNS);
        const query = `
