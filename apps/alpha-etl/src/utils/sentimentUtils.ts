...
function findMatchingClassification(input: string): (typeof SENTIMENT_CLASSIFICATIONS)[number] | undefined {
  const normalized = input.toLowerCase().trim();
  return SENTIMENT_CLASSIFICATIONS.find((option) => option.toLowerCase() === normalized);
}
 ...
 */
export function isValidSentimentClassification(classification: string): boolean {
