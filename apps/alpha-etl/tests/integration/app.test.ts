...
const {
  mockTestDatabaseConnection,
  mockPingDatabase,
  mockStartDatabaseHealthMonitor,
  mockCloseDbPool,
} = vi.hoisted(() => ({
...
const { mockWebhooksRouter, mockHealthRouter } = vi.hoisted(() => {
  ...
  mockWebhooksRouter.post('/pipedream', (_req: Request, res: Response) => {
...
describe('Express Application', () => {
  ...
  describe('middleware functionality', () => {
    ...
    it('should handle large JSON payloads within limit', async () => {
      ...
      const response = await request(app).get('/').send(largeData);
  ...
  describe('routing', () => {
    ...
    it('should handle 404 for non-existent routes with query parameters', async () => {
      const response = await request(app).get(
        '/non-existent?param=value&other=123',
      );
  ...
  describe('error handling', () => {
    ...
    it('should handle async errors', async () => {
      ...
      originalApp.get('/test-async-error', async () => {
    ...
    it('should provide request ID in error responses', async () => {
      ...
      originalApp.get('/test-error-with-id', () => {
  ...
  describe('server lifecycle', () => {
    ...
    it('should handle startServer database connection errors', async () => {
      mockTestDatabaseConnection.mockRejectedValueOnce(
    ...
    it('should not auto-start server when imported (ESM entry check)', () => {
