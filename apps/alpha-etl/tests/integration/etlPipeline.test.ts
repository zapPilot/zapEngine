...
import { testScenarios, expectedTransformations, testUtils } from '../fixtures/fixtures.js';
...
const { PoolDataTransformer } = await import('../../src/modules/pool/transformer.js');
...
describe('ETL Pipeline Integration (Core Tests)', () => {
  ...
  describe('Data Transformation', () => {
    it('should properly transform DeFiLlama APY to APR in pipeline', async () => {
      ...
      const transformed = transformer.transform(testScenarios.defillamaHighYield);
      ...
      testUtils.expectValidSymbolParsing(
        transformed,
        ['wmatic', 'trumatic']
      );
    });
  ...
  describe('Multi-Source Processing', () => {
    it('should process hyperliquid source successfully', async () => {
      ...
      expect(result.sourceResults.hyperliquid.recordsInserted).toBeGreaterThan(0);
    });
