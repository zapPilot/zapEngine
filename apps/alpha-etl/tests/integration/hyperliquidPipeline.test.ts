...
vi.mock('../../src/modules/wallet/portfolioWriter.js', () => ({
  PortfolioItemWriter: vi.fn().mockImplementation(function PortfolioItemWriter() {
    return mockPortfolioWriter;
  }),
}));

vi.mock('../../src/modules/hyperliquid/aprWriter.js', () => ({
  HyperliquidVaultAprWriter: vi.fn().mockImplementation(function HyperliquidVaultAprWriter() {
 ...
 */
const createMockVaultDetailsResponse = (overrides: Partial<VaultDetailsResponse> = {}): VaultDetailsResponse => ({
...
describe('Hyperliquid Pipeline Integration Tests', () => {
  ...
  describe('Complete E2E Flow - Happy Path', () => {
    it('should successfully process multiple VIP users through entire pipeline', async () => {
      ...
      // Mock 3 VIP users
      const vipUsers = [
        createMockVipUser('user-1', '0xuser1111111111111111111111111111111111111'),
        createMockVipUser('user-2', '0xuser2222222222222222222222222222222222222'),
        createMockVipUser('user-3', '0xuser3333333333333333333333333333333333333'),
      ];
      ...
      vaultResponses.forEach((response, index) => {
        mockHyperliquidFetcher.extractPositionData
          .mockReturnValueOnce({
        ...
        mockHyperliquidFetcher.extractAprData
          .mockReturnValueOnce({
            vaultAddress: response.vaultAddress,
            vaultName: response.name || 'HLP Vault',
            leaderAddress: response.leader,
      ...
      expect(mockHyperliquidFetcher.extractPositionData).toHaveBeenCalledTimes(3);
      ...
      const portfolioWriteCall = mockPortfolioWriter.writeSnapshots.mock.calls[0][0];
    ...
    it('should verify data consistency through transformation pipeline', async () => {
      ...
      const vipUsers = [createMockVipUser('user-1', '0xuser1111111111111111111111111111111111111')];
      mockSupabaseFetcher.fetchVipUsers.mockResolvedValue(vipUsers);
      ...
      const portfolioRecords = mockPortfolioWriter.writeSnapshots.mock.calls[0][0];
      expect(portfolioRecords).toHaveLength(1);

      const portfolioRecord = portfolioRecords[0];
      expect(portfolioRecord.wallet).toBe('0xuser1111111111111111111111111111111111111');
      ...
      expect(portfolioRecord.detail).toEqual(expect.objectContaining({
      ...
      expect(portfolioRecord.asset_token_list[0]).toEqual(expect.objectContaining({
  ...
  describe('Partial Failure Scenarios', () => {
    it('should handle mixed success/failure for individual users', async () => {
      ...
      mockHyperliquidFetcher.getVaultDetails
        .mockResolvedValueOnce(vaultResponse1);
      mockHyperliquidFetcher.extractPositionData
        .mockReturnValueOnce({
          userWallet: vipUsers[0].wallet,
      ...
      // User 2: API failure
      mockHyperliquidFetcher.getVaultDetails
        .mockRejectedValueOnce(new Error('Hyperliquid API timeout for user-2'));
      ...
      mockHyperliquidFetcher.getVaultDetails
        .mockResolvedValueOnce(vaultResponse3);
      mockHyperliquidFetcher.extractPositionData
        .mockReturnValueOnce({
  ...
  describe('APR Deduplication', () => {
    it('should deduplicate APR snapshots when multiple users share same vault', async () => {
      ...
      vipUsers.forEach((user, index) => {
        ...
        mockHyperliquidFetcher.getVaultDetails.mockResolvedValueOnce(vaultResponse);
    ...
    it('should keep latest APR snapshot when vault address matches', async () => {
      ...
      const olderTime = new Date('2024-01-01T10:00:00Z').toISOString();
      const newerTime = new Date('2024-01-01T11:00:00Z').toISOString();
      ...
      mockHyperliquidFetcher.getVaultDetails.mockResolvedValueOnce(vaultResponse1);
      mockHyperliquidFetcher.extractPositionData.mockReturnValueOnce({
        userWallet: vipUsers[0].wallet,
        vaultAddress: sharedVault,
        vaultName: 'HLP Vault',
      ...
      mockHyperliquidFetcher.getVaultDetails.mockResolvedValueOnce(vaultResponse2);
      mockHyperliquidFetcher.extractPositionData.mockReturnValueOnce({
  ...
  describe('Concurrent User Processing', () => {
    it('should process 10 users sequentially', async () => {
      ...
      const vipUsers = Array.from({ length: 10 }, (_, i) =>
        createMockVipUser(`user-${i + 1}`, `0xuser${(i + 1).toString().padStart(40, '0')}`)
      );
      mockSupabaseFetcher.fetchVipUsers.mockResolvedValue(vipUsers);
