...
describe("Fear & Greed Pipeline", () => {
  describe("FearGreedFetcher", () => {
    ...
    it("fetchCurrentSentiment should throw on API error code", async () => {
      ...
      await expect(fetcher.fetchCurrentSentiment()).rejects.toThrow(
        "CoinMarketCap API error (code 1001)",
      );
    });
  ...
  describe("SentimentDataTransformer", () => {
    ...
    it("resolveClassificationRange should fall back to full range for unknown classification", () => {
      const range = (transformer as unknown).resolveClassificationRange(
        "UNKNOWN_CLASS",
      );
  ...
  describe("SentimentETLProcessor", () => {
    ...
    it("process should run successfully", async () => {
      ...
      // Mock writer
      vi.spyOn(
        SentimentWriter.prototype,
        "writeSentimentSnapshots",
      ).mockResolvedValue({
        success: true,
        recordsInserted: 1,
        errors: [],
    ...
    it("process should handle transform failure (null return)", async () => {
      ...
      // Mock transformer
      vi.spyOn(SentimentDataTransformer.prototype, "transform").mockReturnValue(
        null,
      );
    ...
    it("process should handle transform exception", async () => {
      ...
      vi.spyOn(
        SentimentDataTransformer.prototype,
        "transform",
      ).mockImplementation(() => {
    ...
    it("healthCheck should return status", async () => {
      vi.spyOn(FearGreedFetcher.prototype, "healthCheck").mockResolvedValue({
        status: "healthy",
      });
