...
describe('Backfill Router', () => {
    ...
    describe('POST /backfill', () => {
        it('should process valid backfill request successfully', async () => {
            ...
            const payload = {
                tokens: [
                    { tokenId: 'bitcoin', tokenSymbol: 'BTC', daysBack: 30 }
                ],
                trigger: 'manual'
            };
        ...
        it('should handle partial failures (some tokens fail)', async () => {
            ...
            expect(response.body.data.results[1].error.message).toBe('API rate limited');
        });
        ...
        it('should fail token result when DMA fails after retries', async () => {
            ...
            mockProcessor.updateDmaForToken.mockRejectedValue(new Error('DMA permanently failed'));
        ...
        it('should handle non-Error exception in processor', async () => {
            mockProcessor.backfillHistory.mockRejectedValue('String error without Error type');
