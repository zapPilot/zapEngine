...
const mockMacroFearGreedProcessor = {
  backfillHistory: vi.fn(),
};
...
vi.mock('../../../src/modules/macro-fear-greed/index.js', () => ({
  MacroFearGreedETLProcessor: vi.fn(function MacroFearGreedETLProcessor() {
...
describe('Backfill Router', () => {
  ...
  describe('POST /backfill/macro-fear-greed', () => {
    it('should backfill CNN macro Fear & Greed history', async () => {
      mockMacroFearGreedProcessor.backfillHistory.mockResolvedValue({
        requested: 1200,
        existing: 100,
        fetched: 1200,
        inserted: 1100,
      ...
      expect(mockMacroFearGreedProcessor.backfillHistory).toHaveBeenCalledWith(
        '2021-01-01',
      );
      expect(response.body.success).toBe(true);
    ...
    it('should reject invalid macro Fear & Greed start date', async () => {
      await request(app)
        .post('/backfill/macro-fear-greed')
