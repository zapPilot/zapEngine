...
import type { ETLJob, ETLJobResult, WebhookPayload } from '../../../src/types/index.js';
...
describe('Webhooks Router', () => {
  ...
  const createMockSuccessResult = (overrides: Partial<SuccessResultData> = {}): ETLJobResult => {
    return {
      success: true,
      data: {
        jobId: 'job-123',
  ...
  const createMockFailureResult = (overrides: Partial<FailedResultError> = {}): ETLJobResult => {
    return {
      success: false,
      error: {
        code: 'API_ERROR',
  ...
  describe('GET /webhooks/jobs/:jobId', () => {
    ...
    it('should return partial success with errors as 206', async () => {
      ...
      const result = createMockSuccessResult({
        ...
        sourceResults: {
          defillama: { errors: ['Warning: deprecated API used'], success: true }
        }
      });
  ...
  describe('TypeScript Type Safety', () => {
    it('should handle valid DataSource union types', async () => {
      ...
      const mockJob = createMockJob({ sources: ['defillama', 'debank', 'hyperliquid'] });
      mockJobQueue.enqueue.mockResolvedValueOnce(mockJob);
