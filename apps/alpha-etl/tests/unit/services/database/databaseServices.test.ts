...
describe('Database Services', () => {
    ...
    describe('PortfolioItemWriter', () => {
        it('should handle database error in writeSnapshots', async () => {
            ...
            const result = await writer.writeSnapshots([{ wallet: 'w1', id_raw: 'id1' } as unknown]);
            expect(result.success).toBe(false);
    ...
    describe('TokenPriceWriter', () => {
        it('should handle error in insertBatch', async () => {
            ...
            vi.spyOn(writer as any, 'withDatabaseClient').mockImplementation(async (fn: unknown) => {
                return await (fn as (client: unknown) => Promise<unknown>)({ query: mockClient.query });
            });
            mockClient.query.mockRejectedValue(new Error('Insert error'));

            await expect(writer.insertBatch([{
                timestamp: new Date(),
                priceUsd: 100,
                marketCapUsd: 1000,
                volume24hUsd: 500,
