...
describe('HyperliquidVaultAprWriter', () => {
  ...
  const createSnapshot = (overrides: Partial<HyperliquidVaultAprSnapshotInsert> = {}): HyperliquidVaultAprSnapshotInsert => ({
    source: 'hyperliquid',
  ...
  beforeEach(async () => {
    ...
    const { HyperliquidVaultAprWriter } = await import('../../../../src/modules/hyperliquid/aprWriter.js');
  ...
  describe('Empty and Single Record Operations', () => {
    ...
    it('should insert a single record successfully', async () => {
      ...
      expect(sql).toContain('INSERT INTO alpha_raw.hyperliquid_vault_apr_snapshots');
      expect(sql).toContain('ON CONFLICT (vault_address, snapshot_time) DO UPDATE');
  ...
  describe('Batch Processing', () => {
    it('should process 50 records in a single batch', async () => {
      const snapshots = Array.from({ length: 50 }, (_, i) =>
        createSnapshot({ vault_address: `0x${i.toString().padStart(40, '0')}` })
      );
  ...
  describe('SQL Query Generation', () => {
    ...
    it('should prevent SQL injection via parameterized queries', async () => {
      ...
      expect(values[1]).toBe("'; DROP TABLE hyperliquid_vault_apr_snapshots; --");
