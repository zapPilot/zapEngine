...
function mockWithClient(
  ...
  withDatabaseClientSpy.mockImplementation(
    async (operation: WithDatabaseClientOperation) => {
...
describe("PortfolioItemWriter", () => {
  ...
  beforeEach(async () => {
    ...
    const { PortfolioItemWriter } =
      await import("../../../../src/modules/wallet/portfolioWriter.js");
    writer = new PortfolioItemWriter();

    // Spy on withDatabaseClient to intercept database operations
    withDatabaseClientSpy = vi.spyOn(
      writer as unknown as object,
      "withDatabaseClient",
    );
  });
  ...
  describe("Required Field Validation", () => {
    ...
    it("should reject record missing id_raw", async () => {
      ...
      expect(result.errors[0]).toContain(
        "Invalid portfolio snapshot encountered",
      );
  ...
  describe("JSONB Serialization", () => {
    it("should serialize all JSONB fields using JSON.stringify", async () => {
      ...
      expect(values[11]).toBe(
        JSON.stringify([{ symbol: "HLP" }, { symbol: "USDC" }]),
      ); // asset_token_list
  ...
  describe("SQL Query Generation", () => {
    it("should generate parameterized query with all columns", async () => {
      ...
      expect(values[4]).toBe(
        "hyperliquid_vault_0xdfc24b077bc1425ad1dea75bcb6f8158e10df303",
      ); // id_raw
    });
    ...
    it("should log validation warnings with wallet context", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          "Invalid portfolio snapshot encountered for wallet",
        ),
      );
    });
  });
