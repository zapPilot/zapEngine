...
import { DeFiLlamaFetcher, type DeFiLlamaResponse, type DeFiLlamaPool } from '../../../../src/modules/pool/fetcher.js';
import { fixturesData } from '../../../fixtures/fixtures.js';
import { createMockApiResponse, mockCurrentTime, restoreRealTime } from '../../../utils/testHelpers.js';
...
describe('DeFiLlamaFetcher', () => {
  ...
  describe('healthCheck', () => {
    it('returns unhealthy when fetch fails', async () => {
      ...
      vi.spyOn(fetcher, 'fetchPoolsByChain').mockRejectedValueOnce(new Error('boom'));
  ...
  describe('fetchAllPools', () => {
    ...
    const createValidPool = (overrides: Partial<DeFiLlamaPool> = {}): DeFiLlamaPool => ({
      pool: 'pool-' + Math.random().toString(36).substring(7),
      chain: 'Polygon',
      project: 'balancer-v2',
      symbol: 'USDC-DAI',
      tvlUsd: 1000000,
    ...
    describe('successful API responses', () => {
      it('should fetch and transform valid pools', async () => {
        const mockData = {
          status: 'success',
          data: [
            createValidPool({ pool: 'pool-1', chain: 'Polygon', project: 'balancer-v2', tvlUsd: 1000000 }),
            createValidPool({ pool: 'pool-2', chain: 'Ethereum', project: 'compound', tvlUsd: 2000000 }),
          ],
        };
      ...
      it('should apply TVL threshold filtering', async () => {
        ...
        expect(result.every(pool => (pool.tvl_usd || 0) > tvlThreshold)).toBe(true);
      });
      ...
      it('should transform pool data correctly', async () => {
        ...
        expect(transformed.tvl_usd).toBe(1500000.50);
    ...
    describe('rate limiting', () => {
      ...
      it('should track request statistics', async () => {
        const mockResponse = createMockApiResponse({ status: 'success', data: [] });
        getMockFetch().mockResolvedValueOnce(mockResponse);
  ...
  describe('fetchPoolsByChain', () => {
    ...
    const createValidMockPool = (chain: string, tvlUsd: number): DeFiLlamaPool => ({
      pool: `pool-${chain}-${Math.random().toString(36).substring(7)}`,
      chain,
      project: 'compound-v2',
      symbol: 'USDC',
      tvlUsd,
  ...
  describe('findMatchingPool', () => {
    it('should find pools matching all criteria', async () => {
      const mockResponse = createMockApiResponse(fixturesData.defillama as DeFiLlamaResponse);
      getMockFetch().mockResolvedValueOnce(mockResponse);
  ...
  describe('Additional edge cases', () => {
    ...
    describe('matchesProject edge cases', () => {
      it('should reject pool when version does not match protocol (line 210)', async () => {
        ...
        const result = await fetcher.findMatchingPool('ethereum', 'compound', 'v3', ['comp', 'eth']);
    ...
    describe('matchesSymbols edge cases', () => {
      it('should return false when matching a pool with an empty normalized symbol', () => {
        const fetcherAny = fetcher as unknown as {
          matchesSymbols(pool: { symbol: string }, targetSymbols: string[]): boolean;
        };

        const result = fetcherAny.matchesSymbols({ symbol: '' }, ['eth', 'usdc']);
      ...
      it('should skip pools with empty symbols during symbol matching', async () => {
        ...
        const result = await fetcher.findMatchingPool('ethereum', 'test-protocol', '0', ['eth', 'usdc']);
      ...
      it('should fall back to loose symbol matching when strict matching fails (line 221)', async () => {
        ...
        const result = await fetcher.findMatchingPool('ethereum', 'test-protocol', '0', ['dai', 'usdc', 'weth']);
    ...
    describe('healthCheck error handling', () => {
      it('should handle API errors during health check (lines 248-258)', async () => {
        ...
        vi.spyOn(testFetcher, 'fetchPoolsByChain').mockRejectedValueOnce(new Error('API unavailable'));
      ...
      it('should handle non-Error exceptions during health check', async () => {
        ...
        const testFetcher = new DeFiLlamaFetcher();
        vi.spyOn(testFetcher, 'fetchPoolsByChain').mockRejectedValueOnce('String error');
    ...
    describe('Explicit code paths', () => {
      it('should continue loop when chain mismatch (line 133)', async () => {
        ...
        const result = await fetcher.findMatchingPool('Ethereum', 'uniswap', '0', ['ETH', 'USDC']);
