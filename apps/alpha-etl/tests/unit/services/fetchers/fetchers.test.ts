...
class TestFetcher extends BaseApiFetcher {
    ...
    getSourceType() { return 'test'; }
    getStats() { return {}; }
    async testGet() { return this.fetchJson('/test'); }
    async testPost(data: unknown) {
        return this.fetchJson('/test', { headers: { 'Method': 'POST' }, body: JSON.stringify(data) } as unknown);
    }
    healthCheck() { return Promise.resolve({ status: 'healthy' } as unknown); }
}
...
describe('Fetchers', () => {
    ...
    describe('BaseApiFetcher', () => {
        ...
        it('should handle non-ok response', async () => {
            ...
            await expect(fetcher.testGet()).rejects.toThrow('500 Internal Server Error');
        });
        ...
        it('should handle fetchWithRetry default parameters', async () => {
            ...
            const result = await fetcher.fetchWithRetry('https://api.example.com/test');
  ...
  return mockLogger();
        ...
        it('should handle error in fetchVipUsersWithActivity', async () => {
            vi.spyOn(fetcher as unknown, 'withDatabaseClient').mockRejectedValue(new Error('DB Failed'));

            await expect(fetcher.fetchVipUsersWithActivity()).rejects.toThrow('DB fetch with activity failed: DB Failed');
        });

        it('should handle error in batchUpdatePortfolioTimestamps', async () => {
            vi.spyOn(fetcher as unknown, 'withDatabaseClient').mockRejectedValue(new Error('Update Fail'));
    ...
    describe('DeFiLlamaFetcher', () => {
        ...
        it('should handle error in findMatchingPool', async () => {
            vi.spyOn(fetcher as unknown, 'fetchAllPools').mockRejectedValue(new Error('Search Failed'));

            const result = await fetcher.findMatchingPool('eth', 'proj', '1', ['TEST']);
            expect(result).toBeNull();
        });

        it('should handle error in fetchAllPools', async () => {
            vi.spyOn(fetcher as unknown, 'fetchDeFiLlamaJson').mockRejectedValue(new Error('Fetch Failed'));
