...
describe('HyperliquidFetcher', () => {
  ...
  describe('getVaultDetails', () => {
    ...
    it('handles response with all optional fields missing (minimal valid)', async () => {
      ...
      expect(result.vaultAddress).toBe(
  ...
  describe('getVaultDetailsForUsers', () => {
    ...
    it('throws when all users fail', async () => {
      ...
      await expect(fetcher.getVaultDetailsForUsers(users)).rejects.toThrow(
        APIError,
      );
      ...
      const vaultDetails = createValidVaultResponse({
        ...
        portfolio: [
          [
            'week',
            {
              accountValueHistory: [[1000000, 50000]],
            },
          ],
        ],
      });
      ...
      const vaultDetails = createValidVaultResponse({
        ...
        portfolio: [
          [
            'day',
            {
              accountValueHistory: [[1000000, NaN]],
            },
          ],
        ],
      });
  ...
  describe('Edge Cases', () => {
    it('should throw "All retry attempts failed" when maxRetries is 0 (line 420)', async () => {
      ...
      await expect(
        zeroRetryFetcher.getVaultDetails('0x123', '0x456'),
    ...
    it('should return null for invalid string TVL in deriveTvlFromPortfolio (line 475)', async () => {
      const badPortfolio = [
        [
          'day',
          {
            accountValueHistory: [[1234567890, 'not-a-number']],
          },
        ],
      ];
    ...
    it('should handle non-Error exceptions in getVaultDetails (line 244)', async () => {
      ...
      await expect(fetcher.getVaultDetails('0x123')).rejects.toBe(
        'String Error',
      );
    ...
    it('should handle non-Error exceptions in getVaultDetailsForUsers (line 285)', async () => {
      ...
      await expect(fetcher.getVaultDetailsForUsers(['0x123'])).rejects.toThrow(
        /All vault detail fetches failed/,
      );

      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to fetch vault details for user'),
        expect.objectContaining({
          error: 'Unknown error',
        }),
      );
  ...
  describe('fetchWithRetryAndBackoff', () => {
    ...
    it('retries with exponential backoff after failures', async () => {
      const fetcher3Retries = new HyperliquidFetcher({
        maxRetries: 3,
        retryDelayMs: 10,
      });
    ...
    it('throws after max retries exhausted', async () => {
      const fetcher2Retries = new HyperliquidFetcher({
        maxRetries: 2,
        retryDelayMs: 0,
      });
      ...
      await expect(fetcher2Retries.getVaultDetails('0xuser')).rejects.toThrow(
        'Persistent failure',
      );
  ...
  describe('HyperliquidFetcher constructor', () => {
    it('creates a HyperliquidFetcher instance', async () => {
      ...
      expect(fetcher.getDefaultVaultAddress()).toBe(
        '0xdfc24b077bc1425ad1dea75bcb6f8158e10df303',
      );
    });
  });
