...
describe('ETLJobQueue', () => {
  ...
  describe('Queue Status', () => {
    ...
    it('should report pending, processing, completed, and failed jobs', async () => {
      ...
      privateQueue(jobQueue).jobs.set('failed', { jobId: 'failed', status: 'failed' });
      privateQueue(jobQueue).jobs.set('completed', { jobId: 'completed', status: 'completed' });
  ...
  describe('Basic Job Processing', () => {
    ...
    it('should handle job processing failure', async () => {
      ...
      mockProcessor.processJob.mockRejectedValue(new Error('Processing failed'));
    ...
    it('should process multiple jobs sequentially', async () => {
      mockProcessor.processJob.mockImplementation(async () => {
  ...
  describe('persistJobStatus', () => {
    ...
    it('should skip persistence when metadata is undefined', async () => {
      await privateQueue(jobQueue).persistJobStatus('job-id', 'pending', undefined);
    ...
    it('should persist job status for wallet_fetch jobs with userId', async () => {
      ...
      expect(mockDbClient.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining(['job-123', 'user-456', '0x1234567890abcdef', 'pending'])
      );
    });
    ...
    it('should handle database errors gracefully (non-fatal)', async () => {
      vi.mocked(getDbClient).mockRejectedValue(new Error('DB connection failed'));
  ...
  describe('MV refresh integration', () => {
    ...
    it('should trigger MV refresh when job has records inserted', async () => {
      ...
      expect(mvRefresher.refreshAllViews).toHaveBeenCalledWith('job-with-records');
    });
    ...
    it('should trigger MV refresh for wallet_fetch jobs even with 0 records', async () => {
      ...
      expect(mvRefresher.refreshAllViews).toHaveBeenCalledWith('wallet-job-zero');
    });
    ...
    it('should trigger MV refresh when partial ETL failure still inserted records', async () => {
      ...
      expect(mvRefresher.refreshAllViews).toHaveBeenCalledWith('partial-failure-with-inserts');
      expect(job.status).toBe('failed');
    ...
    it('should mark job as failed when MV refresh fails', async () => {
      ...
      if (result) {
        expect(result.data.status).toBe('failed');
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshSuccess).toBe(false);
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshFailedCount).toBe(1);
      }
    });
    ...
    it('should mark job as failed when MV refresh throws non-Error', async () => {
      ...
      vi.mocked(mvRefresher.refreshAllViews).mockRejectedValue('MV refresh failed');
    ...
    it('should include MV refresh stats in job result', async () => {
      ...
      expect(result?.success).toBe(true);
      if (result?.success) {
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshDurationMs).toBe(2000);
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshSuccess).toBe(true);
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshFailedCount).toBe(0);
        expect(castTo<Record<string, unknown>>(result.data).mvRefreshResults).toHaveLength(2);
      }
    });
  });
});
