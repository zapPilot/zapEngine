...
import type { BaseETLProcessor, ETLProcessResult } from '../../../../src/core/processors/baseETLProcessor.js';
...
describe('ETLPipelineFactory', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { ETLPipelineFactory } = await import('../../../../src/modules/core/pipelineFactory.js');
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with all supported processors', () => {
      ...
      expect(supportedSources).toEqual(
        expect.arrayContaining(['defillama', 'debank', 'hyperliquid', 'feargreed'])
      );
      expect(supportedSources).toHaveLength(5);
    });
    ...
    it('should create processor instances during initialization', () => {
      ...
      expect(factory.getProcessor('hyperliquid')).toBe(mockHyperliquidProcessor);
    ...
    it('should allow injecting a custom registry', async () => {
      class CustomProcessor implements BaseETLProcessor {
        async process(_job: ETLJob): Promise<ETLProcessResult> {
          return {
            success: true,
            recordsProcessed: 0,
            recordsInserted: 0,
      ...
      expect(customFactory.getProcessor('defillama')).toBeInstanceOf(CustomProcessor);
  ...
  describe('processJob', () => {
    ...
    it('should handle unexpected errors in processJob', async () => {
      const job = createMockJob();
      const processSourceSpy = vi.spyOn(factory as unknown, 'processSource').mockRejectedValue('Unexpected failure');
    ...
    it('should handle processor returning unsuccessful result', async () => {
      ...
      expect(result.errors).toEqual(['defillama: Validation failed for 3 records']);
    ...
    it('should handle unexpected exception during final result processing and trigger main catch block', async () => {
      ...
      const job = createMockJob();
      const processingError = new Error('Unexpected error during result aggregation');
      ...
      const fromEntriesSpy = vi.spyOn(Object, 'fromEntries').mockImplementationOnce(() => {
  ...
  describe('processSingleSource', () => {
    it('should handle Error instances thrown by processor', async () => {
      ...
      const result = await (factory as unknown).processSingleSource(job, processor);
  ...
  describe('healthCheck', () => {
    ...
    it('should handle non-Error exceptions during health check', async () => {
      ...
      mockWalletProcessor.healthCheck = vi.fn().mockRejectedValue('String error');
      mockHyperliquidProcessor.healthCheck = vi.fn().mockResolvedValue({
        status: 'healthy'
      });
      mockSentimentProcessor.healthCheck = vi.fn().mockResolvedValue({
  ...
  describe('getStats', () => {
    it('should collect stats from all processors successfully', () => {
      ...
      mockHyperliquidProcessor.getStats = vi.fn().mockReturnValue(hyperliquidStats);
    ...
    it('should handle processor getStats errors gracefully', () => {
      ...
      mockHyperliquidProcessor.getStats = vi.fn().mockReturnValue({ requestsTotal: 0 });
      mockSentimentProcessor.getStats = vi.fn().mockReturnValue({ requestsTotal: 1 });
  ...
  describe('getSupportedSources', () => {
    it('should return array of supported data sources', () => {
      ...
      expect(sources).toEqual(['defillama', 'debank', 'hyperliquid', 'feargreed', 'token-price']);
  ...
  describe('Advanced TypeScript Patterns and Edge Cases', () => {
    ...
    it('should handle concurrent processor operations', async () => {
      ...
      // Should process sequentially, not concurrently (as per current implementation)
      expect(endTime - startTime).toBeGreaterThan(140); // ~150ms total
      expect(result.success).toBe(true);
    });
