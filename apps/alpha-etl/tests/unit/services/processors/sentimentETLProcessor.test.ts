...
vi.mock('../../../../src/modules/sentiment/index.js', async () => {
  const actualModule = await vi.importActual<typeof import('../../../../src/modules/sentiment/index.js')>('../../../../src/modules/sentiment/index.js');
...
describe('SentimentETLProcessor', () => {
  ...
  it('processes sentiment data successfully', async () => {
    const rawData = { value: 60, classification: 'Greed', timestamp: 1_700_000_000, source: 'coinmarketcap' };
    const transformed = {
      sentiment_value: 60,
    ...
    expect(mockWriter.writeSentimentSnapshots).toHaveBeenCalledWith([transformed], 'feargreed');
  });
  ...
  it('returns fetcher stats', () => {
    mockFetcher.getRequestStats.mockReturnValue({ requestCount: 2, lastRequestTime: 123 });

    const stats = processor.getStats();

    expect(stats).toEqual({ feargreed: { requestCount: 2, lastRequestTime: 123 } });
  });
  ...
  describe('API Error Handling', () => {
    it('handles 401 Unauthorized (invalid API key)', async () => {
      ...
      expect(result.recordsInserted).toBe(0);
      expect(result.errors).toContain('Failed to fetch sentiment data from API');
      expect(mockWriter.writeSentimentSnapshots).not.toHaveBeenCalled();
    });
    ...
    it('handles 500 Internal Server Error', async () => {
      const serverError = new Error('CoinMarketCap API error (code 500): Unknown error');
    ...
    it('includes error details in result.errors array', async () => {
      const detailedError = new Error('CoinMarketCap API error: Service unavailable - maintenance window');
  ...
  describe('Health Check Scenarios', () => {
    ...
    it('includes detailed error message in health check', async () => {
      mockFetcher.healthCheck.mockResolvedValue({
        status: 'unhealthy',
        details: 'CoinMarketCap API error: 503 Service Unavailable - Maintenance in progress'
      });
  ...
  describe('Database Constraint Violations', () => {
    ...
    it('reports constraint errors without failing job', async () => {
      ...
      mockWriter.writeSentimentSnapshots.mockResolvedValue({
        success: false,
        recordsInserted: 0,
        errors: ['Unique constraint violation: duplicate key value violates unique constraint "sentiment_snapshots_source_snapshot_time_key"'],
        duplicatesSkipped: 0
      });
