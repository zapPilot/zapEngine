...
vi.mock('../../../../src/modules/vip-users/supabaseFetcher.js', () => ({
  SupabaseFetcher: class {
    ...
    batchUpdatePortfolioTimestamps = mockSupabaseFetcher.batchUpdatePortfolioTimestamps;
...
describe('WalletBalanceETLProcessor', () => {
  ...
  describe('process', () => {
    ...
    it('should handle errors during processing', async () => {
      const job = createMockJob();
      mockSupabaseFetcher.fetchVipUsers.mockRejectedValueOnce(new Error('Database error'));
    ...
    it('should continue processing other users if one user fails', async () => {
      ...
      const job = createMockJob();
      const vipUsers = [
        { user_id: 'user-success', wallet: '0x1234567890123456789012345678901234567890' },
        { user_id: 'user-fail', wallet: '0xabcdefabcdefabcdefabcdefabcdefabcdefabcd' },
      ];
      ...
      mockDeBankFetcher.fetchComplexProtocolList
        .mockResolvedValueOnce([]);
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to fetch data for user', {
        jobId: job.jobId,
        userId: 'user-fail',
        wallet: '0xabcd...abcd',
      ...
      expect(mockTransformer.transformBatch).toHaveBeenCalledWith([
        expect.objectContaining({ user_wallet_address: '0x1234567890123456789012345678901234567890' })
      ]);
    });
    ...
    it('should handle cases where transformation results in no data', async () => {
      ...
      mockSupabaseFetcher.fetchVipUsers.mockResolvedValue([
        { user_id: 'user1', wallet: '0x1234567890123456789012345678901234567890' }
      ]);
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('No valid data after wallet balance transformation', {
        jobId: job.jobId
      });
  ...
  describe('healthCheck', () => {
    ...
    it('should return unhealthy when a service is unhealthy', async () => {
      mockDeBankFetcher.healthCheck.mockResolvedValueOnce({ status: 'unhealthy', details: 'API error' });
    ...
    it('should include unhealthy status when DeBank details are missing', async () => {
      mockDeBankFetcher.healthCheck.mockResolvedValueOnce({ status: 'unhealthy' });
    ...
    it('should include Supabase details when Supabase is unhealthy', async () => {
      mockSupabaseFetcher.healthCheck.mockResolvedValueOnce({ status: 'unhealthy', details: 'DB timeout' });
    ...
    it('should handle health check errors with Error instance', async () => {
      mockDeBankFetcher.healthCheck.mockRejectedValueOnce(new Error('Health check failed'));
    ...
    it('should handle health check errors with non-Error values', async () => {
      mockDeBankFetcher.healthCheck.mockRejectedValueOnce('Health check failed');
  ...
  describe('internal helpers', () => {
    it('should skip undefined balances and portfolio items during aggregation', async () => {
      ...
      const processUserWalletSpy = vi.spyOn(processorAny, 'processUserWallet').mockResolvedValue({
        success: true,
        balances: undefined,
    ...
    it('should surface unknown errors from fetchUserData', async () => {
      const processorAny = processor as unknown;
      const fetchUserDataSpy = vi.spyOn(processorAny, 'fetchUserData').mockRejectedValue('Fetch failed');
    ...
    it('should return a fetch failure when fetchUserData resolves to null', async () => {
      const processorAny = processor as unknown;
      const fetchUserDataSpy = vi.spyOn(processorAny, 'fetchUserData').mockResolvedValue(null);
  ...
  describe('Activity-based filtering integration', () => {
    it('should skip inactive users with recent updates', async () => {
      ...
      const inactiveUser = {
        ...
        last_portfolio_update_at: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
      };
    ...
    it('should include active users in processing', async () => {
      const activeUser = {
        ...
        last_portfolio_update_at: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(),
      };
      ...
      expect(mockDeBankFetcher.fetchWalletTokenList).toHaveBeenCalledWith('0xACTIVE');
      expect(mockDeBankFetcher.fetchComplexProtocolList).toHaveBeenCalledWith('0xACTIVE');
    });
    ...
    it('should update inactive users after 7+ days', async () => {
      const staleUser = {
        ...
        last_portfolio_update_at: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString(),
      };
      ...
      expect(mockDeBankFetcher.fetchWalletTokenList).toHaveBeenCalledWith('0xSTALE');
      expect(mockDeBankFetcher.fetchComplexProtocolList).toHaveBeenCalledWith('0xSTALE');
    });
    ...
    it('should handle mixed user populations', async () => {
      ...
      expect(mockDeBankFetcher.fetchWalletTokenList).toHaveBeenCalledWith('0x1111111111111111111111111111111111111111');
      expect(mockDeBankFetcher.fetchWalletTokenList).toHaveBeenCalledWith('0x3333333333333333333333333333333333333333');
      expect(mockDeBankFetcher.fetchWalletTokenList).not.toHaveBeenCalledWith('0x2222222222222222222222222222222222222222');
    });
  ...
  describe('Portfolio timestamp updates', () => {
    it('should call batchUpdatePortfolioTimestamps for successful wallets', async () => {
      const users = [
        { user_id: 'u1', wallet: '0xWALLET1111111111111111111111111111111111', last_activity_at: null, last_portfolio_update_at: null },
        { user_id: 'u2', wallet: '0xWALLET2222222222222222222222222222222222', last_activity_at: null, last_portfolio_update_at: null },
      ];
    ...
    it('should handle timestamp update failure gracefully', async () => {
      const users = [{ user_id: 'u1', wallet: '0xWALLET1111111111111111111111111111111111', last_activity_at: null, last_portfolio_update_at: null }];
      ...
      mockDeBankFetcher.fetchWalletTokenList.mockResolvedValue([
        { id: '0xtoken1', chain: 'eth', name: 'TestToken', symbol: 'TEST', amount: 100, price: 1 }
      ]);
      mockDeBankFetcher.fetchComplexProtocolList.mockResolvedValue([]);

      // Set up transformer and writer mocks
      mockTransformer.transformBatch.mockReturnValue([
        { user_wallet_address: '0xWALLET1111111111111111111111111111111111', token_address: '0xtoken1', chain: 'eth', symbol: 'TEST', amount: 100 }
      ]);
      ...
      const users = [
        {
          ...
          wallet: '0x1111111111111111111111111111111111111111',
          last_activity_at: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(),
          last_portfolio_update_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        },
      ];
      ...
      const users = [
        {
          ...
          last_portfolio_update_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
        },
      ];
