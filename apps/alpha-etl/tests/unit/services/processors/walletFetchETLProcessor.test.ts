...
describe('WalletFetchETLProcessor', () => {
  ...
  describe('process()', () => {
    it('should successfully process a wallet fetch job', async () => {
      ...
      mockTransformer.transformBatch.mockReturnValue([
        {
          user_wallet_address: job.metadata?.walletAddress,
          token_address: 'eth',
          amount: 1.5,
        },
        {
    ...
    it('should handle DeBank fetch failure gracefully', async () => {
      ...
      mockDebankFetcher.fetchWalletTokenList.mockRejectedValue(
    ...
    it('should handle invalid DeBank response', async () => {
      ...
      expect(result.recordsProcessed).toBe(0);
      expect(result.errors).toContain(
        'Failed to fetch DeBank data for 0x1234...7890',
      );
  ...
  describe('healthCheck()', () => {
    ...
    it('should handle health check errors', async () => {
      mockDebankFetcher.healthCheck.mockRejectedValue(
