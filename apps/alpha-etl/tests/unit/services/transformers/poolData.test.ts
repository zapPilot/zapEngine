  ...
  describe("transform", () => {
    describe("valid data transformation", () => {
      it("should transform DeFiLlama data correctly", () => {
        ...
        expect(result!.tvl_usd).toBe(1500000.5);
      ...
      it("should transform all provided test cases correctly", () => {
        validPoolDataSamples.forEach((sample) => {
    ...
    describe("symbol parsing", () => {
      ...
      it("should handle complex symbol patterns", () => {
        ...
        complexSymbols.forEach((symbol) => {
          const data = createPoolDataBuilder().withSymbol(symbol).build();
    ...
    it("should handle extreme numerical values", () => {
      ...
      extremeCases.forEach(({ apy, expected, tvl_usd }, index) => {
        ...
        if (expected === null) {
          expect(
            result,
            `Case ${index}: apy=${apy}, tvl_usd=${tvl_usd} should be rejected`,
          ).toBeNull();
        } else if (expected === 0) {
          expect(
            result,
            `Case ${index}: apy=${apy} should give APR=0`,
          ).not.toBeNull();
          expect(result!.apr).toBe(0);
        } else {
          expect(
            result,
            `Case ${index}: apy=${apy} should be accepted`,
          ).not.toBeNull();
        }
      });
    });
