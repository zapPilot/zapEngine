  ...
  describe('HyperliquidDataTransformer', () => {
    ...
    it('transformPosition returns null if validation fails', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        'Hyperliquid position contains invalid numeric values',
        expect.anything(),
      );
    });
    ...
    it('transformPosition catches unexpected errors', () => {
      ...
      vi.spyOn(transformer as unknown, 'createSnapshot').mockImplementation(
        () => {
    ...
    it('transformApr throws error on invalid APR', () => {
      expect(() =>
        transformer.transformApr({ apr: Infinity, vaultAddress: '0x1' }, {}),
      ).toThrow('Invalid APR value');
    });
  ...
  describe('BaseTransformer', () => {
    it('transformBatchWithLogging processes items and logs stats', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Test items batch transformation completed',
        {
          total: 3,
          success: 1,
          errors: 2,
