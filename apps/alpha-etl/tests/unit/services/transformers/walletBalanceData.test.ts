...
describe("WalletBalanceTransformer", () => {
  ...
  const createMockWalletBalanceData = (
    overrides: Partial<WalletBalanceData> = {},
  ): WalletBalanceData => ({
    user_id: "user1",
  ...
  describe("performance considerations", () => {
    it("should handle concurrent transformations without interference", async () => {
      const rawData1 = createMockWalletBalanceData({
        user_id: "user1",
        symbol: "ETH",
      });
      const rawData2 = createMockWalletBalanceData({
        user_id: "user2",
        symbol: "USDC",
      });
  ...
  describe("additional edge cases", () => {
    ...
    it("should handle protocol_id variations", () => {
      ...
      testCases.forEach((testCase) => {
        const rawData = createMockWalletBalanceData({
          protocol_id: testCase.protocol_id as unknown,
        });
        const result = transformer.transform(rawData);
    ...
    it("should handle time_at edge cases", () => {
      ...
      testCases.forEach((testCase) => {
        const rawData = createMockWalletBalanceData({
          time_at: testCase.time_at as unknown,
        });
        const result = transformer.transform(rawData);
