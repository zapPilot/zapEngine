...
describe('Utils', () => {
  ...
  describe('SymbolParser', () => {
    ...
    it('should handle perfect match with underlying tokens', () => {
      ...
      expect(parseSymbolsArray('ETH-USDC', ['token1', 'token2'])).toEqual([
        'ETH',
        'USDC',
      ]);
      expect(parseSymbolsArray('WETH-USDT-DAI', ['t1', 't2', 't3'])).toEqual([
        'WETH',
        'USDT',
        'DAI',
      ]);
    ...
    it('should handle excess parts with duplicates - lines 46-56', () => {
      ...
      expect(parseSymbolsArray('ETH-ETH-USDC', ['token1', 'token2'])).toEqual([
        'ETH',
        'USDC',
      ]);
    ...
    it('should handle excess parts without duplicates match', () => {
      ...
      expect(parseSymbolsArray('ETH-USDC-DAI', ['token1', 'token2'])).toEqual([
        'ETH',
        'USDC',
        'DAI',
      ]);
    ...
    it('should handle fewer parts (hyphenated tokens) - lines 59-66', () => {
      ...
      expect(parseSymbolsArray('WSTETH-ETH', ['t1', 't2', 't3'])).toEqual([
        'WSTETH',
        'ETH',
      ]);
    });
