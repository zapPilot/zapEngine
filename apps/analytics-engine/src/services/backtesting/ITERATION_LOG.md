...
### 2026-09-22 - Research-only Laya direct-allocation backtest

- **Status**: Research infrastructure + first 500-day production-history measurement. No strategy registry entry, saved config, API contract, default rule, production allocation path, validation fixture, or pinned snapshot was changed.
- **Implementation**: `scripts/research/laya/` adds a compact causal observation, typed Laya question presets, deterministic decoders, 5pp allocation snapping, a research decision policy, persistent JSONL response cache, fake client, composed-strategy wrapper, and one runner that evaluates both production baselines plus three Laya experiment encodings against the same prepared dataset and engine invocation. `laya>=0.3.5` lives in the `research` dependency group; the real model import is lazy, and importing the research package does not load `laya`, `torch`, or `transformers`.
- **Encodings**: `per_bucket_score` asks four parallel score questions and defaults to expected-score decoding; `posture_mixture` asks one five-way posture choice and mixes the full returned probability distribution into risk-off/defensive/balanced/growth/risk-on templates; `static_equal_weight` is a model-free 25/25/25/25 control. All targets normalize to stable/SPY/BTC/ETH, snap to 5 percentage-point units, and unchanged targets HOLD. Decision cadence is configurable.
- **Pinned-window run**: real `convaiinnovations/laya` English checkpoint on Apple MPS, reference date 2026-09-20, same 500-day production-history window as `strategy_performance_snapshot_500d.json`, expected-score decoding, daily cadence. Baselines reproduced the snapshot guardrails: `dma_fgi_portfolio_rules` ROI **88.94%**, Sharpe **2.75**, Sortino **3.72**, Calmar **6.64**, MaxDD **-8.93%**, 64 trades; `dca_classic` ROI **-6.33%**, Sharpe **-0.01**, MaxDD **-39.62%**, 499 trades.
- **Laya result**: `per_bucket_score` ROI **13.09%**, Sharpe **0.49**, Sortino **0.71**, Calmar **0.30**, MaxDD **-31.25%**, 2 trades, annualized turnover **0.409x**, 2 distinct snapped targets. `posture_mixture` ROI **16.16%**, Sharpe **0.72**, Sortino **1.10**, Calmar **0.56**, MaxDD **-20.88%**, 127 trades, annualized turnover **6.117x**, 7 distinct snapped targets. `static_equal_weight` ROI **12.21%**, Sharpe **0.46**, Sortino **0.67**, Calmar **0.27**, MaxDD **-32.48%**, 1 trade, annualized turnover **0.333x**.
- **Finding**: This first result is negative evidence for replacing the canonical rule-based strategy. Both Laya variants materially trail it on absolute and risk-adjusted performance. `posture_mixture` does beat the equal-weight control on this window, while `per_bucket_score` is only marginally better and collapses to two snapped targets. No promotion claim is justified from one historical window.
- **Reproducibility**: the first real-model run was interrupted by the execution harness after 300s with 536 decisions already persisted; rerunning the identical command resumed from that cache and completed the remaining decisions. This validates the intended resumable cache path under an actual long model run.
- **Validation**: Laya-specific suite `26 passed`; analytics-engine Turbo lint + type-check clean; fake-client 500-day run reproduced both baselines before the real model run. The production snapshot fixture was not regenerated because production strategy behavior is unchanged.
- **Next**: vary cadence and checkpoint/encoding, then evaluate disjoint or walk-forward windows. Do not register Laya as a production strategy unless it first demonstrates robust out-of-sample improvement over both the equal-weight control and `dma_fgi_portfolio_rules`.

### 2026-09-05 - Causal technical signal snapshot + 12 non-default rule experiments

- **Status**: Research capability only. No default rule, priority, saved config, registry entry, API contract, allocation wire type, landing-page artifact, or validation fixture changed. `DEFAULT_PORTFOLIO_RULES` is byte-identical.
- **Commit**: pending local change (`feat/technical-signal-experiments`).
- **Finding**: `DmaMarketState` gained a typed `technical` snapshot derived per asset from the trailing close history the strategy context already carries (RSI(14) + 5-day slope, annualized 20-day realized volatility, 30/90-day momentum, MACD(12,26,9) with histogram crosses, 20-day Bollinger z-score, prior-20-day channel breaks, and a trailing two-window RSI divergence proxy). Twelve non-default rules consume it and are addressable through `enabled_rules`; each attaches a `technical_signals` diagnostic for the assets that triggered it. Priorities 60-71 sit numerically above every default rule, so a `default + experiment` run keeps canonical decisions first. Indicators read a fixed 260-bar trailing window (`TECHNICAL_LOOKBACK_BARS`) instead of the whole run, so a calendar day's values no longer depend on the backtest start date; over a 500-day 3-asset run that is 0.46s vs 0.74s of indicator work, with the recursive seeds converged to ~3e-7 absolute on RSI and ~6e-9 relative on MACD. The rules are declared as one table over a shared buy/sell pair rather than one class per indicator, and the shared above-DMA symbol filter moved into `portfolio_rules/base.py` next to `symbols_for_snapshot`.
- **Snapshot delta**: None. `DEFAULT_PORTFOLIO_RULES` and the default decision path are unchanged, `DmaMarketState.technical` is not serialized into the snapshot, and no `--update-snapshot` was performed. `test:strategy-snapshot:fast` was **not run locally** (it needs `DATABASE_READ_ONLY_URL`); the CI `tests` job runs it.
- **Validation**: `ruff format --check src tests`, `ruff check src tests`, `mypy src`, `vulture src vulture_whitelist.py --min-confidence 80`, `pylint --rcfile=pyproject.toml src/`, `node ../../scripts/lint/run-jscpd.mjs src` (0 clones), and `scripts/ci/run-tests-precommit.sh --coverage`. `signals/technical.py` and `portfolio_rules/technical_experiments.py` are both at 100% line coverage. Behavioral evidence lives in `tests/services/backtesting/signals/test_technical.py` (trailing-window invariance, unusable-price rejection, one MACD cross per trend turn), `test_flat_minimum.py` (three assets get three distinct per-asset technical states through the real signal component), and `tests/services/backtesting/portfolio_rules/test_technical_experiments.py` (an enabled technical rule drives a `RuleBasedPortfolioStrategy` decision end to end, and the default strategy selects none of them against a snapshot where every technical flag fires).
- **Next**: No indicator is promoted and no default behavior is proposed. Before promoting one, run its isolated `enabled_rules` sweep per `ITERATION_PLAYBOOK.md` and compare ROI, Sharpe, Calmar, MaxDD, trade count, and behavior traces — not ROI alone. ATR / ADX / Stochastic / OBV / VWAP stay out until real OHLCV reaches the strategy context.
...
- **Removed**:
  ...
  - `sweep_production_window.py`: 6 entries from `METRIC_KEYS` / `DEFAULT_TOLERANCES`, their `TOLERANCE_ALIASES`, and the 6 `_metric` writers in `_snapshot_strategy_entry`. **KEPT** the 3 core metrics added alongside in 2026-05-29 (`sortino_ratio` / `volatility` / `ulcer_index`) — legitimate core metrics, not part of the six.
  - 500-day snapshot fixture: 6 keys from `tolerances` + both strategy blocks; no DB regen needed (removal touches no retained value). Synced to `apps/landing-page/src/data/strategy-snapshot.json` via `lint:snapshot-sync:fix`.
  - Zod (`packages/types/src/strategy/backtesting.ts::BacktestStrategySummarySchema`) 6 `z.number().optional()` fields; `scripts/contracts/snapshots/backtest_response.json` regenerated via `contracts:export`.
- **Re-add path if ever needed**: the registry pattern makes each metric one file (~30 LOC) + one tuple entry — cheap, documented in the 2026-05-28 entry.
- **Validation in this environment**: `ruff check src tests scripts` clean; `mypy src` 212 files clean; `pytest tests/services/backtesting/execution tests/scripts/{test_sweep_production_window,test_optuna_search}.py` 202 passed; `test:strategy-snapshot:fast` → "Checked metrics: 10 / Drift rows: 0" (was 22 — the six gone, retained anchors byte-identical); `pnpm contracts:check` → "Contract parity checks passed"; `turbo type-check` types + frontend + landing-page green.
...
- **KEPT (accurate names)**: the `DmaGatedFgi*` family (`DmaGatedFgiParams`, `DmaGatedFgiConfig`, `DmaGatedFgiSignalComponent`, `DmaGatedFgiSignalRuntime`, `DmaGatedFgiPublicParams`) and `signals/dma_gated_fgi/` — these name the specific DMA-cross × FGI-gate **signal** (one rule subset), which is exactly what they are.
- **Validation**: `ruff check`/`mypy src` clean (220 files); broad pytest sweep 850 passed / 3 skipped; wire-id string count unchanged (115 → 115 before/after the rename).
...
- **Validation in this environment**:
  ...
  - `uv run pytest` (affected: registry/presets/config-store/v3/snapshot-coverage) → 56 passed, 3 skipped; broad sweep (`tests/services/backtesting tests/config tests/api/test_backtesting.py …`) → 835 passed
  - `tests/integration/test_strategy_performance_snapshot.py::test_every_registered_strategy_has_snapshot_entry` → passes (universe == {`dca_classic`, `dma_fgi_portfolio_rules`} == snapshot keys)
  - `pnpm lint:snapshot-sync` → in sync. **Not run locally**: `test:strategy-snapshot:fast` (needs `DATABASE_READ_ONLY_URL`; the two kept strategies' numbers are unchanged, so the CI gate stays green).
...
- **Spot-check values for `dma_fgi_portfolio_rules`** (the canonical baseline) on the 500-day window ending 2026-04-15:
  - `omega_ratio = 1.9242` (winners exceed losers by ~2× in expected magnitude — consistent with the 2.63 Sharpe)
  - `tail_ratio = 1.3264` (the right tail is ~33% larger than the left)
  - `skewness = 2.3994` (heavy right-tail; matches the strategy's risk-on regime captures)
  - `excess_kurtosis = 15.5032` (very heavy-tailed daily return distribution)
  - `pain_index = 0.9790` (low; drawdowns are short relative to total time)
  - `max_drawdown_recovery_days = 17.0` (worst drawdown was reabsorbed in 17 bars)
  - These are now byte-identical regression anchors. Future strategy edits that move them should land a new ITERATION_LOG entry explaining why.
- **New probe**: `scripts/attribution/extended_metrics_rank_compare.py` (~265 LOC, 21 unit tests, no DB). Reads the fixture and computes Spearman ρ between each new metric and each existing risk metric (sharpe / sortino / calmar / max_drawdown / volatility / ulcer_index) across the strategy universe, plus the 6×6 internal redundancy matrix. Heuristic cutoffs: `|ρ_max_with_existing| < 0.7` → NEW INFO; `0.7 ≤ |ρ_max| ≤ 0.9` → PARTIAL OVERLAP; `|ρ_max| > 0.9` → DUPLICATE. Output: `reports/extended_metrics_rank_correlation.md`.
- **First probe verdicts — STATISTICALLY VOID at n=3**: the report's verdict block (`omega_ratio` DUPLICATE, `tail_ratio` DUPLICATE, `skewness` DUPLICATE, `excess_kurtosis` NEW INFO, `pain_index` DUPLICATE, `max_drawdown_recovery_days` PARTIAL OVERLAP) is **not informative**. With only 3 strategies, ρ can take at most 6 distinct values, and the 3 strategies happen to be near-monotonically ordered across every existing metric (good ones are good on every axis, bad ones are bad on every axis). Almost any new metric reading produces |ρ|≈1 with sharpe/sortino/calmar/max_dd/volatility/ulcer_index — that means rank-saturation, not redundancy. The probe report banners this explicitly via the `_distinct_rho_count(n)` warning block; verdicts only become actionable once n ≥ ~10.
- **Validation in this environment**:
  - `uv run ruff check / ruff format / mypy src` → clean
  - `uv run pytest tests/services/backtesting/execution/metrics/ tests/scripts/test_extended_metrics_rank_compare.py` → 71 pass (50 metric units + 21 probe units)
  - `pnpm contracts:check` → parity holds (Zod ↔ Pydantic synced)
  - `pnpm --filter @zapengine/analytics-engine run test:strategy-snapshot:fast` → "Checked metrics: 22 / Drift rows: 0"
  - `tests/integration/test_strategy_performance_snapshot.py` → 3/3 pass (including the previously-failing `fixed_interval_rebalance` coverage test)
...
- **Phase E verification (Pending → done)**: first real Optuna run — 30 TPE trials on `dma_fgi_portfolio_rules`, full range 2024-01-01 → 2026-04-15, default walk-forward (in-sample 180d / out-of-sample 60d / step 30d, sliding). Report: `apps/analytics-engine/reports/optuna_first_run.json`.
  - **Best**: trial #28, `oos_sharpe_mean = 1.853`. Params: `cross_cooldown_days = 78`, `cross_on_touch = True`, `pacing_k = 16.58`, `pacing_r_max = 1.69`, `buy_sideways_window_days = 13`, `buy_sideways_max_range = 0.0997`, `dma_overextension_threshold = 0.364`, `overextension_threshold_multiplier_greed = 0.204`, `overextension_threshold_multiplier_extreme_greed = 0.317`, `fgi_slope_reversal_threshold = -0.197`, `fgi_slope_recovery_threshold = 0.131`.
  - **Primary sanity pass**: `cross_cooldown_days = 78` is mid-upper, not the searched floor 7. The explicitly-feared overfit-to-rapid-reentry attractor was avoided. Worst trial #6 (`oos_sharpe_mean = 1.607`) has `cross_cooldown_days = 7`, confirming the dose-response direction.
  - **Secondary signal — search-bound saturation**: top 6 trials all in `cross_cooldown_days ∈ [78, 90]`; three trials pinned at the upper bound 90 (#12, #13, #21). TPE believes longer cooldown improves OOS Sharpe than the search range allows.
  - **Diagnostic gap in the report shape**: JSON records `best_value` (= `oos_sharpe_mean`) but not `in_sample_sharpe_mean` per trial, so the in/oos gap — the load-bearing overfit guard from `WalkForwardReport` — cannot be read off the JSON. The runtime computes it; `optuna_search.py` does not propagate it.
- **Conclusion on the 6 new extended metrics — UNCONSUMED**: the Phase A/B registry (`OmegaRatio`, `TailRatio`, `Skewness`, `ExcessKurtosis`, `PainIndex`, `MaxDrawdownRecoveryTime`) is intentionally not folded into `PerformanceMetricsCalculator.calculate_all_metrics()` to keep the 500-day fixture byte-stable. No attribution flow, no saved-config flow, no decision policy, and not even this Optuna search invokes `compute_extended_metrics()`. **Are they useful as a decision signal? Not yet measured.** The plumbing is correct (50 unit tests pass on synthetic arrays) but the experiment that would actually demonstrate usefulness — running them across the strategy universe and showing the rankings differ from `Sharpe / Sortino / Calmar / MaxDD` — has not been run. Treat Phase A/B as infrastructure landed, not value delivered. Calling the registry "effective" or "ineffective" right now is premature.
- **Followups (priority order)**:
  1. **Optuna search-bound revisit**: extend `cross_cooldown_days` upper bound from 90 to 180 and re-run 30+ trials. If the new winner is again pinned at the new upper bound, the cooldown rule is doing duty for a different effect (likely regime-hold) and should be re-modeled rather than re-tuned.
  2. **Propagate `in_sample_sharpe_mean` through the Optuna report** (~10 LOC in `optuna_search.py`): record `in_sample_sharpe_mean` and `oos_in_sample_gap` per trial so overfit checks don't require a separate `WalkForwardRunner` rerun.
  3. **Extended-metrics attribution probe**: one-off script `scripts/attribution/extended_metrics_rank_compare.py` that calls `compute_extended_metrics()` on each strategy's daily returns and values, then reports Spearman rank correlation vs existing metrics. If any new metric has `|ρ| < 0.7` with all existing metrics, it carries new information and is a candidate to fold into snapshots. If all new metrics are `|ρ| > 0.9` with at least one existing metric, the registry duplicated existing knowledge and Phase A/B has not delivered.
  4. **Pre-existing `fixed_interval_rebalance` snapshot coverage drift**: `tests/integration/test_strategy_performance_snapshot.py::test_every_registered_strategy_has_snapshot_entry` fails because `fixed_interval_rebalance` (added in PR#26) was registered but never added to the fixture. Independent fix via `sweep_production_window.py --update-snapshot`.
- **Per-rule report / rule-only sweep**: not run — no default rule, priority, trigger, or production-default behavior changed in this iteration.
...
- **Validation in this environment (no Supabase access)**:
  - Phase D: `uv run ruff check src/services/backtesting/validation/ tests/services/backtesting/validation/` + `uv run mypy src/services/backtesting/validation/walk_forward.py` clean. `uv run pytest tests/services/backtesting/validation/` → 25 pass (15 new + 10 existing event_runner).
  - Phase E: `uv run ruff check scripts/attribution/optuna_search.py tests/scripts/test_optuna_search.py` + `uv run mypy scripts/attribution/optuna_search.py` clean. `uv run pytest tests/scripts/test_optuna_search.py` → 10 pass.
  - Combined regression: `uv run pytest tests/services/backtesting/ tests/scripts/test_optuna_search.py --ignore=tests/test_strategy_performance_snapshot.py` → 789 pass.
- **Pending local verification by author** (requires `DATABASE_READ_ONLY_URL`):
  - Real walk-forward run: `pnpm --filter @zapengine/analytics-engine exec uv run python -c "import asyncio, datetime as dt; from src.core.database import init_database, session_scope; from src.services.shared.query_service import QueryService; from src.services.market.token_price_service import TokenPriceService; from src.services.market.sentiment_database_service import SentimentDatabaseService; from src.services.market.stock_price_service import StockPriceService; from src.services.market.macro_fear_greed_service import MacroFearGreedDatabaseService; from src.services.strategy.backtesting_service import BacktestingService; from src.services.strategy.strategy_config_store import StrategyConfigStore; from src.services.backtesting.validation.walk_forward import WalkForwardConfig, WalkForwardRunner; init_database(); ...; print(report)"` (or use Phase E script — easier).
  - Real Optuna run (30 trials): `pnpm --filter @zapengine/analytics-engine exec uv run python scripts/attribution/optuna_search.py --strategy-id dma_fgi_portfolio_rules --start-date 2024-01-01 --end-date 2026-04-15 --trials 30 --output reports/optuna_first_run.json`. **Sanity check**: best_params with `cross_cooldown_days=1` (or any very short value) should NOT be the winner if the harness genuinely penalises overfit.
- **Per-rule report / rule-only sweep**: not run — no default rule, priority, trigger, or production-default behavior changed in this iteration.
- **Next**:
