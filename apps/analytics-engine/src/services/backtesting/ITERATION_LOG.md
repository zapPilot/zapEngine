...
### 2026-06-01 - First Optuna walk-forward run reproducible on `main`; tuned params land as a selectable research config (NOT promoted)
- **Status**: Walk-forward Optuna parameter search confirmed runnable on `main` (the harness was already complete — the earlier "first real run" lived on an unmerged branch and its `reports/` output never reached `main`). The best trial is a **null result against the promote gate**; it ships as a non-default, user-selectable comparison config, not as the production default.
- **Correction to prior framing**: `src/services/backtesting/validation/walk_forward.py` is fully implemented on `main` (the `WalkForwardRunner._run_fold` path calls `run_compare_v3` and aggregates per-fold metrics). No harness code change was needed. The DB-gated run just needed `DATABASE_READ_ONLY_URL`.
- **The run** (`scripts/attribution/optuna_search.py`, study `dma_fgi_wf_2026_05_31`, 30 TPE trials, seed 42, in-sample 180d / out-of-sample 60d / step 30d sliding over 2024-01-01..2026-04-15, against the Supabase read-only replica): best **trial #13, `oos_sharpe_mean = 1.8122`** vs the default config's **1.7455** on the same harness (**+3.8%**), with a near-zero overfit gap (`in_sample_sharpe_mean 1.8192 − oos 1.8122 = 0.0070`, vs default's 0.0594). Transient replica connection drops were ridden over via a resume-until-30 loop on a sqlite study (`load_if_exists`); the loop is pure orchestration, no code change. Root-cause fix for the drops (`pool_pre_ping=True` in `src/core/database.py`) was deliberately **not** taken — it touches shared production DB config and was out of scope.
- **Best params (trial #13)**: `cross_cooldown_days=90` (pinned at the search upper bound — saturation, same signal the 2026-05-28 entry flagged at bound 90), `cross_on_touch=False`, `pacing_k=1.5188`, `pacing_r_max=1.1968`, `buy_sideways_window_days=14`, `buy_sideways_max_range=0.07408`, `dma_overextension_threshold=0.38055`, `overextension_threshold_multiplier_greed=0.48080`, `overextension_threshold_multiplier_extreme_greed=0.20148`, `fgi_slope_reversal_threshold=-0.25777`, `fgi_slope_recovery_threshold=0.06535`.
- **Promote gate — FAILS (null result)**: on the full pinned 500-day window (`2024-12-02..2026-04-15`, BTC, 10000), tuned vs `dma_fgi_portfolio_rules_default` (via `saved_config_id`): ROI **76.4111 vs 76.8826 (−0.47pp)**, Sharpe 2.6197 vs 2.6274, Calmar 5.7509 vs 5.7841, MaxDD −8.9255 vs −8.9254, trades 54 vs 53. Every full-window metric is marginally **worse**. The walk-forward OOS Sharpe edge does not carry to the full window, so the production default is **unchanged**.
- **What shipped instead — a selectable research config (additive, no default change)**: new non-default seed `dma_fgi_portfolio_rules_optimized` in `src/config/strategy_presets.py` (display name "DMA/FGI Portfolio Rules (Optimized)"), reusing `strategy_id=dma_fgi_portfolio_rules` so the snapshot universe (keyed on `strategy_id` via `list_strategy_recipes()`) is unchanged and the 500-day fixture stays byte-stable. Mechanism: `_ComposedPresetDefinition` gained an optional `public_params_override`; `_resolve_public_params` / `_resolve_params_model` overlay it on the strategy defaults via the existing `_merge_public_params`. `supports_daily_suggestion=False` (research only). Verified end-to-end: `POST /api/v3/backtesting/compare` with `saved_config_id=dma_fgi_portfolio_rules_optimized` reproduces ROI 76.4111 — i.e. the composition path honours all nested knobs (pacing / buy_gate / top_escape), not just the two projected signal fields.
- **Frontend (so it's selectable in the dashboard)**: `BacktestingView` strategy dropdown switched from the strategy-family catalog (`strategies`, deduped by `strategy_id`) to the preset catalog (`presets`, by `config_id`), so two presets sharing a `strategy_id` are each selectable. Touches `useBacktestConfiguration.ts` (`parseSelectedConfigId` + preset-keyed `strategyOptions`), `jsonConfigurationHelpers.ts` (`parseConfigStrategyIdWithPresets` → `parseSelectedConfigId`), `backtestConfigurationBuilders.ts` (new `buildCompareConfigForConfigId`), `BacktestTerminalDisplay.tsx` + `BacktestingView.tsx` (`selectedStrategyId` → `selectedConfigId`). Browser-verified at `/?tab=invest&invest=backtesting`: the dropdown lists both "DMA/FGI Portfolio Rules" and "DMA/FGI Portfolio Rules (Optimized)", and selecting + running the optimized config drives a distinct backtest (chart legend `dma fgi portfolio rules optimized`).
- **Validation in this environment**:
  - Backend: `uv run ruff check` / `ruff format --check` / `mypy src/config/strategy_presets.py` clean; `uv run pytest tests/config/ tests/services/test_strategy_config_store.py tests/services/test_strategy_daily_suggestion_service.py tests/services/test_strategy_config_management_service.py tests/integration/test_strategy_performance_snapshot.py` → 75 pass / 3 skipped (snapshot-coverage stays green — universe unchanged). API tests under `tests/api/` ERROR pre-existing on missing local pg (port 5433); confirmed identical with my change stashed.
  - Frontend: `turbo run lint type-check --filter=@zapengine/frontend` clean; `vitest run` backtesting dir → 365 pass (incl. 7 new tests for `parseSelectedConfigId` + `buildCompareConfigForConfigId`).
  - **Not run locally**: `test:strategy-snapshot:fast` byte-check (the two production strategies' numbers are unchanged; new config is non-default and `strategy_id`-aliased, so the CI gate stays green).
- **Per-rule report / rule-only sweep**: not applicable — no default rule, priority, trigger, or production-default behavior changed; this iteration only tunes continuous params on a *non-default* config and adds UI plumbing.
- **Followups**: (1) the cooldown upper-bound saturation (pinned at 90) is still open from 2026-05-28 — a run with the bound raised to 180 would tell whether `cross_cooldown_days` is proxying a regime-hold effect; not done here (would change the search space and break direct comparability with this run). (2) If a future window makes the tuned params actually clear the full-window gate, promotion is a one-line flip of `is_default` plus a snapshot regen.
