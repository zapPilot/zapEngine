...
class PortfolioBuckets:
    ...
    def to_portfolio(
        ...
        return Portfolio.from_asset_values(
            ...
            spy_value=float(0.0 if self.spy_value is None else self.spy_value),
...
def map_portfolio_to_spy_eth_btc_stable_buckets(portfolio: Any) -> PortfolioBuckets:
    ...
    SPY exposure is read from ``allocation.spy.total_value`` if present; otherwise
    treated as zero so the strategy still functions before the upstream payload is
    augmented with SPY tracking.
    """
    ...
    btc_value = float(getattr(getattr(allocation, "btc", None), "total_value", 0.0))
    eth_value = float(getattr(getattr(allocation, "eth", None), "total_value", 0.0))
    spy_value = float(getattr(getattr(allocation, "spy", None), "total_value", 0.0))
    stable_category_value = float(
        getattr(getattr(allocation, "stablecoins", None), "total_value", 0.0)
    )
    alt_value = float(getattr(getattr(allocation, "others", None), "total_value", 0.0))
    stable_value = stable_category_value + alt_value
    return PortfolioBuckets(
        spot_value=btc_value + eth_value + spy_value,
        stable_value=stable_value,
        btc_value=btc_value,
        eth_value=eth_value,
        spy_value=spy_value,
