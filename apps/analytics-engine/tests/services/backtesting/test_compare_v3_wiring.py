...
def test_run_compare_v3_on_data_emits_eth_btc_rotation_asset_timeline() -> None:
    ...
    for state in eth_rotation_points:
        assert state.decision.target_allocation is not None
        assert state.decision.target_allocation.spy == pytest.approx(0.0)

    assert eth_rotation_points[0].decision.rule_group == "rotation"
    assert (
        eth_rotation_points[0].decision.details["matched_rule_name"]
        == "eth_btc_continuous_weight"
    )
    assert eth_rotation_points[1].decision.reason == "regime_no_signal"
    assert (
        eth_rotation_points[1].decision.details["cooldown_skipped_rules"][0]["rule"]
        == "eth_btc_continuous_weight"
    )
    ...
    assert eth_rotation_points[2].decision.target_allocation.btc >= 0.0
    assert eth_rotation_points[2].decision.target_allocation.eth >= 0.0
...
def test_run_compare_v3_on_data_caps_non_cross_eth_btc_stable_buy() -> None:
    ...
    assert "dma_buy_gate" not in buy_day.execution.diagnostics.plugins
    assert buy_day.decision.reason == "portfolio_eth_btc_continuous_weight"
    assert buy_day.decision.details["matched_rule_name"] == "eth_btc_continuous_weight"
    assert buy_day.decision.immediate is False
    assert buy_day.decision.target_allocation is not None
    assert buy_day.decision.target_allocation.stable < 1.0
    assert buy_day.portfolio.stable_usd > 0.0
    assert stable_buy == pytest.approx(0.0)
...
def test_run_compare_v3_on_data_blocks_above_zone_revert_after_ratio_cross_up() -> None:
    ...
    assert cross_day.decision.reason == "portfolio_cross_up_equal_weight"
    assert cross_day.decision.target_allocation is not None
    assert cross_day.decision.target_allocation.btc == pytest.approx(0.5)
    assert cross_day.decision.target_allocation.eth == pytest.approx(0.5)
    assert cross_day.decision.target_allocation.stable == pytest.approx(0.0)
    assert cross_day.signal is not None
    assert cross_day.decision.details["matched_rule_name"] == "cross_up_equal_weight"
    ...
    for blocked_date in (date(2025, 8, 4), date(2025, 8, 5)):
        ...
        assert state.decision.target_allocation is not None
        assert state.decision.target_allocation.btc >= 0.0
...
def test_run_compare_v3_on_data_blocks_below_zone_revert_after_ratio_cross_down() -> (
    ...
    cross_day = eth_rotation_points[date(2025, 8, 3)]
    assert cross_day.decision.reason == "portfolio_eth_btc_continuous_weight"
    assert cross_day.decision.target_allocation is not None
    assert cross_day.decision.target_allocation.eth > (
        cross_day.decision.target_allocation.btc
    )
    assert cross_day.signal is not None
    assert cross_day.decision.details["matched_rule_name"] == "eth_btc_continuous_weight"
...
def test_run_compare_v3_on_data_blocks_eth_btc_rotation_during_cooldown() -> None:
    ...
    first_rotation_day = eth_rotation_points[0]
    blocked_rotation_day = eth_rotation_points[1]

    assert first_rotation_day.decision.rule_group == "rotation"
    assert first_rotation_day.execution.transfers is not None
    assert first_rotation_day.decision.reason == "portfolio_eth_btc_continuous_weight"
    assert blocked_rotation_day.decision.reason == "portfolio_dma_stable_gating"
    assert blocked_rotation_day.execution.transfers is not None
    assert blocked_rotation_day.execution.event == "rebalance"
    ...
    assert blocked_rotation_day.decision.target_allocation.stable > 0.0
    assert any(
        transfer.to_bucket == "stable"
        for transfer in blocked_rotation_day.execution.transfers
    )
    assert (
        blocked_rotation_day.decision.details["cooldown_skipped_rules"][0]["rule"]
        == "eth_btc_continuous_weight"
    )
...
def test_run_compare_v3_on_data_outer_dma_follows_majority_spot_asset() -> None:
    ...
    assert cross_up_day.portfolio.spot_asset is None
    assert cross_up_day.decision.reason == "portfolio_cross_up_equal_weight"
    ...
    assert any(
        transfer.from_bucket == "stable" and transfer.to_bucket in {"btc", "eth"}
        for transfer in cross_up_day.execution.transfers
    )
    assert post_rotation_day.signal is not None
    assert post_rotation_day.portfolio.spot_asset is None
...
def test_run_compare_v3_on_data_does_not_sell_eth_btc_rotation_on_ath_only_days() -> (
    ...
    assert eth_rotation_points[date(2025, 4, 24)].decision.reason in {
    ...
    for sell_date in (date(2025, 4, 28), date(2025, 5, 2), date(2025, 5, 7)):
        ...
        assert state.decision.target_allocation is not None
        assert state.portfolio.asset_allocation.spy == pytest.approx(0.0)
...
def test_run_compare_v3_on_data_only_rotates_back_to_btc_after_ratio_moves_above_dma() -> (
    ...
    assert any(
        state.decision.reason == "portfolio_eth_btc_continuous_weight"
        for state in post_cross_states
    )
