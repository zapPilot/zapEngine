...
The agent backlog is not provider-cached inside the operations service; `force` is accepted on `ops_backlog` for a consistent read-tool shape but does not create a second GitHub fan-out layer.
