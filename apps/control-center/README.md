...
- `/api/mcp` exposes the separately authenticated Ops MCP. Its write capabilities are narrowly allowlisted: bounded agent backlog lifecycle actions on `zapPilot/zapEngine` Issues, and a single-issue Sentry resolve operation, both documented in [`MCP.md`](./MCP.md).
