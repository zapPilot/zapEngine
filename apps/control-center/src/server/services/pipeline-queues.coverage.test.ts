...
describe('pipeline queues coverage', () => {
  it('serves unconfigured without Supabase', async () => {
    ...
    const svc = createPipelineQueuesService({ config: readControlCenterConfig({}), now: () => NOW });
  ...
  it('splits abandoned visuals out of the working render lane', () => {
    ...
    input.visualStates = [
      { episode_id: EP_A, status: 'failed', visual_version: EPISODE_VIDEO_VISUAL_VERSION, abandoned_at: '2026-09-05T05:00:00Z', abandoned_reason: 'nope' },
    ];
  ...
  it('reports stale visual versions as blocked with a stale step', () => {
    const input = base();
    input.visuals = [visual({ status: 'queued', visual_version: 'v0', attempt_count: 0 })];
    const res = buildPipelineQueues(input);
  ...
  it('marks ingest items without an episode row as non-restartable', () => {
    ...
    input.episodes = [];
    input.ingests = [
      { id: 'ing-1', source_url: 'https://example.com/missing', language_code: 'zh-Hant', status: 'failed', attempt_count: 2, lease_owner: null, lease_expires_at: null, last_error: 'boom', created_at: '2026-09-05T03:00:00Z', updated_at: '2026-09-05T04:00:00Z' },
    ];
    const res = buildPipelineQueues(input);
  ...
  it('offers render-local restart only when the shared visual is renderable', () => {
    const input = base();
    input.renders = [
      { episode_localization_id: LOC_A, episode_id: EP_A, status: 'failed', visual_version: EPISODE_VIDEO_VISUAL_VERSION, progress_percent: null, progress_stage: 'Rendering', attempt_count: 2, next_attempt_at: '2026-09-05T04:00:00Z', lease_owner: null, lease_expires_at: null, last_error: 'raster', started_at: null, completed_at: null, thumbnail_url: null, created_at: '2026-09-05T04:00:00Z', updated_at: '2026-09-05T04:30:00Z' },
    ];
    input.visualStates = [
      { episode_id: EP_A, status: 'completed', visual_version: EPISODE_VIDEO_VISUAL_VERSION, abandoned_at: null, abandoned_reason: null },
    ];
    const res = buildPipelineQueues(input);
  ...
  it('derives social publishing/partial/failed from platform states', () => {
    expect(deriveSocialState([{ status: 'publishing' }] as never)).toBe('publishing');
    expect(deriveSocialState([{ status: 'published' }, { status: 'failed' }] as never)).toBe('partial');
    expect(deriveSocialState([{ status: 'failed' }, { status: 'failed' }] as never)).toBe('failed');
    expect(deriveSocialState([{ status: 'failed' }, { status: 'queued' }] as never)).toBe('partial');
    expect(deriveSocialState([{ status: 'queued' }] as never)).toBe('queued');
    expect(deriveSocialState([{ status: 'published' }, { status: 'published' }] as never)).toBe('published');
  });

  it('drops unknown social platforms and sorts history by time', () => {
    const input = base();
    input.socialJobs = [
      { id: 'j1', episode_id: EP_A, platform: 'unknown', language_code: 'en', status: 'queued', scheduled_at: '2026-09-05T07:00:00Z', next_attempt_at: '2026-09-05T07:00:00Z', attempt_count: 1, lease_owner: null, lease_expires_at: null, last_error: null, completed_at: null, created_at: '2026-09-05T06:00:00Z', updated_at: '2026-09-05T06:00:00Z' },
      { id: 'j2', episode_id: EP_A, platform: 'x', language_code: 'en', status: 'failed', scheduled_at: '2026-09-05T05:00:00Z', next_attempt_at: '2026-09-05T05:00:00Z', attempt_count: 2, lease_owner: null, lease_expires_at: null, last_error: 'post boom', created_at: '2026-09-05T04:00:00Z', updated_at: '2026-09-05T05:30:00Z' },
    ];
    input.socialPosts = [];
    const res = buildPipelineQueues(input);
  ...
  it('covers ingest steps and lease-held work items', () => {
    ...
    input.localizations = [];
    input.ingests = [
      { id: 'ing-1', source_url: 'https://example.com/a', language_code: 'zh-Hant', status: 'processing', attempt_count: 1, lease_owner: 'w1', lease_expires_at: '2026-09-05T07:00:00Z', last_error: null, created_at: '2026-09-05T03:00:00Z', updated_at: '2026-09-05T04:00:00Z' },
    ];
    const res = buildPipelineQueues(input);
