...
function okClient() {
  ...
  return {
    from: (table: string) => {
      ...
      chain['order'] = () => chain;
      chain['limit'] = () =>
        Promise.resolve(table === 'social_posts' ? { ...empty } : empty);
...
describe('pipeline queues coverage round 2', () => {
  ...
  it('records completed history and skipped/completed-orphan social states', () => {
    ...
    input.visuals = [
      {
        ...
        completed_at: '2026-09-05T05:00:00Z',
        created_at: '2026-09-05T04:00:00Z',
    ...
    const item =
      res.social.processing[0] ??
      res.social.queued[0] ??
      res.social.attention[0];
    const byPlatform = new Map(
      (item?.platforms ?? []).map((p) => [p.platform, p.status]),
    );
  ...
  it('sorts processing lanes by start time and waiting lanes by availability', () => {
    ...
    input.visuals = [
      {
        ...mkVisual('a', '2026-09-05T05:00:00Z', '2026-09-05T04:00:00Z'),
        episode_id: EP_A,
      },
      {
        ...mkVisual('b', '2026-09-05T04:00:00Z', '2026-09-05T04:00:00Z'),
    ...
    input.socialJobs = [
      {
        ...
        next_attempt_at: '2026-09-05T08:00:00Z',
        attempt_count: 0,
        lease_owner: null,
        lease_expires_at: null,
        last_error: null,
