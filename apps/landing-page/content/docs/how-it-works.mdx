...
import { AssetPairLabel } from '@/components/docs/AssetLabel';
...
When the regime shifts, the rules engine produces a target allocation across the three pillars. Idle capital between rebalances is parked in baseline-yield venues so the portfolio is never doing nothing — but yield is the icing, not the strategy. Current performance figures are backtest results; the live pipeline is still pending.
...
<Tabs items={['EIP-7702 (preferred)', 'Sequential fallback']}>
...
Implementation: `packages/app-core/src/lib/wallet/executeDepositPlan.ts`.
...
### 4. ETH/BTC deviation DCA

When ETH/BTC deviates far from its long-term mean, the engine incrementally
leans toward the relatively cheap side of the pair. The adjustment is gradual
and symmetric: unusually cheap ETH shifts toward ETH, while unusually expensive
ETH shifts toward BTC.

### 5. DMA overextension DCA sell
...
<Callout type="warn" title="What changes vs. what stays">
  ...
  `apps/analytics-engine` and change between releases. The execution model
  (atomic EIP-7702 where supported, sequential wallet transactions otherwise)
  and the signal categories (200MA, Fear & Greed, ETH/BTC RS) are stable.
</Callout>
