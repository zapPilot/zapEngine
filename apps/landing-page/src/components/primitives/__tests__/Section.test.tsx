...
describe('Section', () => {
  it('renders the shared landing section scaffold', () => {
    ...
    expect(section).toHaveClass('shell-section', 'example-section');
  ...
  it('supports section-specific inner classes without changing the outer scaffold', () => {
    const { container } = render(
      <Section className="faq" innerClassName="faq-inner">
        <div>FAQ body</div>
      </Section>,
    );

    expect(container.querySelector('section')).toHaveClass('shell-section');
