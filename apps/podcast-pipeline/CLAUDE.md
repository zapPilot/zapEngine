...
- **`social:daemon` is the canonical social production loop.** It runs on the local Mac because X/Rednote depend on persistent browser sessions. One daemon owns completed-episode discovery, durable enqueue/scheduling, platform publishing, standardized metric snapshots (`1h` / `6h` / `24h` / `72h` / `7d`), and periodic strategy refresh. Do not split metrics or publishing into required companion processes. `social:publish` and `social:metrics` are break-glass/debug commands only. `social:dashboard` is a read-only optional UI and must stay lifecycle-independent so a dashboard port failure can never stop publishing or metrics. Preserve the durable first-start anchor, DB lease/owner fencing, and the rule that missed early metric windows are not backfilled with later observations.
