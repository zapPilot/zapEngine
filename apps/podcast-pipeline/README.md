...
Hono API service for From Fed to Chain. Turns article URLs into multilingual podcast episodes: scrape, OpenRouter LLM script generation, OpenRouter translation (`openrouter/free` first, then shared `LLM_FALLBACK_MODELS`), Fish Audio Text-to-Speech, FFmpeg HLS, Cloudflare R2 upload, and Supabase metadata.
...
Required for full ingest: `OPENROUTER_API_KEY`, `R2_*`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DB_SCHEMA=from_fed_to_chain`, `INGEST_ADMIN_TOKEN`, `FISH_AUDIO_API_KEY`, `FISH_AUDIO_REFERENCE_ID`, and `BRAVE_SEARCH_API_KEY` (the only image-search provider; see [Vertical news video](#vertical-news-video-image-only-multilingual)). Fish Audio is the only TTS provider. Every normal OpenRouter workload uses `LLM_MODEL` as its primary model. Translation is the sole exception: its primary is code-owned `openrouter/free`. All OpenRouter workloads then share the same ordered, comma-separated `LLM_FALLBACK_MODELS` value from `config/env/dev.env` and `config/env/prod.env`; duplicates are removed while preserving order, and an empty list means the primary runs alone. Retryable transport/provider failures (timeout, connection failure, 408/409/429, 5xx, or an HTTP 200 the caller cannot read — `choices` missing, no candidate at all, or a candidate whose text is blank, typically `finish_reason=length` with 0 output characters because the endpoint spent the budget on reasoning tokens) advance through that shared chain, while authentication/configuration/client failures remain terminal. There is no translation-specific fallback list and no translation provider other than OpenRouter.
...
`OPENROUTER_TIMEOUT_MS` limits each ordinary OpenRouter candidate request and defaults to `120000` milliseconds. Invalid or empty values use that default; SDK-level retries stay disabled, and the shared client advances a retryable transport/provider failure to the next model in `LLM_FALLBACK_MODELS`. Workload-owned retry loops run only outside that shared chain. Translation sends canonical scripts longer than 2,000 characters as sequential paragraph/sentence-aware chunks, with the title only on the first chunk, then rejoins the translated chunks with blank lines. Translation additionally retries a response that arrived but is unusable (malformed JSON, a missing or blank field, model chatter), re-prompting with the rejection reason — at `temperature: 0` an identical resend would only reproduce the same output. A transport retry uses fallback routing so OpenRouter can choose a different compatible endpoint; a response-validation retry keeps the normal route. A resubmission still resumes from the latest committed ingest stage.
...
Language-classroom generation is the heaviest call in the pipeline: one response carries a full narration script for every target language. It pins `response_format: json_object` and `reasoning: { enabled: false }` — it is concept selection and writing, not a reasoning task — and deliberately sends **no `max_tokens`**, exactly like the visual-subject catalog call. A ceiling cannot tell a runaway provider from a verbose one: it cuts the body wherever the budget runs out, which in a JSON response means mid-string, turning an answer that only needed re-prompting into one that cannot be parsed at all. A runaway is bounded by the request deadline instead, and a bad body by the retry loop below.

That loop runs up to **three attempts**, on top of the shared per-request transport retry. An attempt is rejected when the response arrived but is unusable: invalid JSON, `finish_reason: length`, a non-object body, no valid lesson at all, or a lesson set that is missing any requested target language. That last rule is the contract, not a nicety: both call sites in `audio-stage.ts` pass exactly the targets they are still missing and hard-fail afterwards if one is absent, so a partial set used to be persisted and then killed two layers up in `assertLanguageClassroomsReady` — past the point where the model could still be asked again. Those failures happen after HTTP 200, so nothing in the transport policy ever sees them — a syntax error 9,112 characters into a 37,164-character body once aborted a whole multilingual ingest and left the episode for a human to resubmit. Every retry drops the deterministic `sort: 'throughput'` so OpenRouter picks a different endpoint, which is where this diverges from translation: a translation validation retry keeps its route because the model wrote a bad field, while here an endpoint returned ten times the expected volume. The re-prompt appends the rejection reason — including which target languages were missing — plus a JSON-escaping and completeness reminder to the unchanged grounding block, so the article and script never drop out of a retry. Rejected attempts were still billed, and their cost is added to the returned `costUsd`. The thrown error carries `provider`, `model`, `finishReason`, `outputChars`, an excerpt around the offending character, and `requested`/`returned`/`accepted` target lists — which is what separates a model that omitted a language from a lesson the parser dropped for a blank script, blank `oneLiner`, or no usable keyword. `llm:retry` records each rejection with `layer: 'payload' | 'transport'` plus a groupable `reason` (`invalid_json`, `truncated`, `not_object`, `no_lessons`, `incomplete_targets`).

All of these are top-level body fields. They previously travelled inside an `extra_body` wrapper, which is the Python SDK's client-side kwarg: that SDK flattens it before the request is sent, so a literal `extra_body` key never reaches OpenRouter and everything inside it was dropped — including `usage: { include: true }`, which is why recorded LLM cost defaulted to zero. A failed request now logs `llm:failed` with the model, input size, timeout, output ceiling and reasoning setting; `llm:response` carries the provider that served it and the `finishReason` it stopped on, and fires whenever a response arrived — including the unusable HTTP 200 that is then rejected as a transport failure, because that line is the only record of which endpoint produced it.

**Script generation is the exception, and it is deliberately not configurable.** Its prompt forbids summarizing, permits an output longer than its input, and sets no token ceiling, so a 13k-character article legitimately generates for minutes. The shared 120 s deadline was therefore killing correct work, and the shared retry then replayed the identical request for another 120 s — one Telegram ingest spent 248 s that way before failing. The step now runs on a private 600 s deadline in `src/services/llm.ts`. That is a constant rather than an environment variable because it is a property of the prompt, not of a deployment. Script generation also pins `reasoning: { enabled: false }`, like language-classroom generation: rewriting an article into narration is not a reasoning task, and an endpoint that spends the output budget on reasoning tokens bills a full run for an empty script.

Within that deadline, each retryable transport/provider failure advances through the same `LLM_FALLBACK_MODELS` chain as every other OpenRouter workload. A timeout that escapes that shared chain is terminal for the script layer: it does not replay the entire model chain for another ten minutes per candidate. A retry-safe gateway/connection failure that still escapes the complete chain keeps the historical single endpoint-reroute attempt with `sort` dropped from `provider`, so the second chain is not deterministically handed back to the same endpoints. The JSON-contract re-prompt is unchanged and is not a transport replay — it carries the rejection reason, so the model is being asked to fix a contract violation rather than to redo work. Language-classroom generation keeps the shared 120-second candidate deadline on purpose: its output is bounded by design, so a candidate still running after two minutes is a degenerate provider and can advance to the next shared fallback model.

Scene alignment for `ja` and `en` uses OpenRouter with the same model policy as the rest of the pipeline: `LLM_MODEL` first, then `LLM_FALLBACK_MODELS` for retryable transport/provider failures. There is no scene-specific model/provider environment switch. If semantic alignment still cannot produce a valid full-coverage mapping, rendering falls back to deterministic proportional alignment so the job remains resumable.
...
`visual:intents enriched=N/M brand=B entities=K subjects=S primarySubject=... model=... cues=N`
reports how much of the episode the catalog covered and how much of it image
search can anchor on a concrete visual subject. `cues` counts the grounded
scene cues and trails at the end because that is the field order the processor
emits; the deterministic path omits it entirely rather than logging
`cues=undefined`. Because the assignment fallback bottoms out at the
...
1. **The publisher's own `og:image` is the first content scene, and the same image is the video cover** (v11). That is the image every share card, feed preview and thumbnail already shows, so the frame a viewer lands on has to be the frame that made them click. The planner hoists the `og:image` to the front of the article-image cursor by reading its `origin`, never the publisher's DOM order, and records what scene 0 actually rendered in `provenance.leadCoverImageUrl`; the render job dresses the cover from that field instead of scraping the article again, which is also what keeps the three language renders of one episode on one image. The decorative filter still judges it — an `og:image` whose URL reads as an icon or a thumbnail is dropped under its named rule into `provenance.leadCoverFallbackReason`, and the lead scene walks the ordinary ladder instead. Viable `og:image`, article/figure images, lazy-load attributes, and largest `srcset` candidates are consumed from the source article before external search, so publisher imagery is used rather than ignored.
...
`selectionMode: 'strict'` (tests and the storyboard smoke CLI) remains the diagnostic path: anchored scenes use the editorial Brave provider and search failures raise rather than entering production reuse or concept-card behavior. `video:storyboard:smoke --script <canonical-script.txt> --title <title> --duration-ms <ms> --output <directory>` writes `draft.json`/`validation-report.json`/`token-usage.json`/`sentences.json`; adding `--catalog [--search-title <title>] [--search-script <english-script.txt>]` additionally runs the catalog + assignment + planner-scene pass and writes `catalog.json`/`assignments.json`/`scene-plan.json` (per-scene `selectionReason`, `visualCue`, `queries`, `cueQuery`, planned requests) without spending Brave budget.
...
**2026-09-21 check-in: opening that second slot on `performance-1x` is not worth it.** Sixteen days of `ops_pipeline_stage_runs` on this shape (2026-09-05 onward, `usage->>'machine' = 'performance-1x-2gb'`) put encode at 81% of render machine-time and the pre-encode window (visual planning, narration download, alignment, upload) at the remaining 19% — about 4.2 machine-hours/day, ~$5.6/month total. Fully overlapping that 19% is the theoretical ceiling of what a second slot buys, and it prices out to roughly $1/month; the realistic gain is smaller, since the first job of a batch has nothing to overlap with yet and the last job's upload has nothing queued behind it. Meanwhile two concurrent encodes measured 2608 MB against this shape's 2 GB with no swap configured — a certain OOM, not a risk — while solo renders over the same 16 days peaked at 1258 MB (p50) / 1558 MB (p95) / 1721 MB (max), zero OOMs. The only way to actually run two encodes at once is `performance-2x`, which the code already supports (the second slot and its `MemFree` guard activate automatically at 2 vCPUs); that halves wall-clock per video at double the rate, so cost per finished video is unchanged and the only thing that moves is the always-on ceiling, from ~$32/month to ~$64/month if the machine ever got stuck awake. Re-open this only if encode's share of machine-time drops below 60% or daily machine-hours exceed 10 — either would mean the pre-encode overlap is worth materially more than $1/month.
