...
Required for full ingest: `OPENROUTER_API_KEY`, `R2_*`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DB_SCHEMA=from_fed_to_chain`, `INGEST_ADMIN_TOKEN`, `FISH_AUDIO_API_KEY`, `FISH_AUDIO_REFERENCE_ID`, and `BRAVE_SEARCH_API_KEY` (the only image-search provider; see [Vertical news video](#vertical-news-video-image-only-multilingual)). Fish Audio is the only TTS provider. Every normal OpenRouter workload uses `LLM_MODEL` as its primary model. Translation is the sole exception: its primary is code-owned `openrouter/free`. All OpenRouter workloads then share the same ordered, comma-separated `LLM_FALLBACK_MODELS` value from `config/env/dev.env` and `config/env/prod.env`; duplicates are removed while preserving order, and an empty list means the primary runs alone. Retryable transport/provider failures (timeout, connection failure, 408/409/429, or 5xx) advance through that shared chain, while authentication/configuration/client failures remain terminal. There is no translation-specific fallback list and no translation provider other than OpenRouter.
