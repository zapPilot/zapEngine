...
5. **Whether an image is about the subject is guaranteed by Brave answering that subject's query, not by the metadata spelling the name.** A candidate mentioning one of the scene's normalized entities in its title/alt text, source-page URL, or image URL only earns a **ranking bonus** — it is never a filter. The hard identity gate this replaced discarded 423 of 423 viable candidates for one episode before a single download, because news photos rarely repeat the subject's name in their alt text.
