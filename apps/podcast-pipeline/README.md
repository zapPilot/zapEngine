...
That loop runs up to **three attempts**, on top of the shared per-request transport retry. An attempt is rejected when the response arrived but is unusable: invalid JSON, `finish_reason: length`, a non-object body, or no valid lesson at all. Those failures happen after HTTP 200, so nothing in the transport policy ever sees them — a syntax error 9,112 characters into a 37,164-character body once aborted a whole multilingual ingest and left the episode for a human to resubmit. Every retry drops the deterministic `sort: 'throughput'` so OpenRouter picks a different endpoint, which is where this diverges from translation: a translation validation retry keeps its route because the model wrote a bad field, while here an endpoint returned ten times the expected volume. The re-prompt appends the rejection reason and a JSON-escaping reminder to the unchanged grounding block, so the article and script never drop out of a retry. Rejected attempts were still billed, and their cost is added to the returned `costUsd`. The thrown error carries `provider`, `model`, `finishReason`, `outputChars` and an excerpt around the offending character, and `llm:retry` records each rejection with `layer: 'payload' | 'transport'` plus a groupable `reason` (`invalid_json`, `truncated`, `not_object`, `no_lessons`).
