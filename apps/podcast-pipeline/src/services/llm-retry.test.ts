...
describe('generateScriptWithLLM request policy', () => {
  ...
  it('does not replay a mixed timeout and gateway-failure model chain', async () => {
    vi.stubEnv('LLM_FALLBACK_MODELS', 'fallback/one');
    const timeout = new APIConnectionTimeoutError({
      message: 'Request timed out.',
    });
    const gateway = providerErrorWithStatus(502);
    const mockCreate = vi
      .fn()
      .mockRejectedValueOnce(timeout)
      .mockRejectedValue(gateway);
    mockOpenAIClient(mockCreate);
