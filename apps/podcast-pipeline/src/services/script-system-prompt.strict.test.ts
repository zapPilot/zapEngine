...
const EXACT_OPENING_RULE = `輸出的第一句必須逐字為：「${EXPECTED_OPENING}」`;
const FIRST_CHARACTER_RULE = '第一個字必須是「各」，首句之前不得輸出任何內容。';
...
describe('script system prompt opening contract', () => {
  it('requires the exact opening before any other generated content', () => {
    ...
    expect(prompt).toContain(EXACT_OPENING_RULE);
    expect(prompt).toContain(FIRST_CHARACTER_RULE);
    expect(prompt).toContain(NO_PREAMBLE_RULE);
  });
});
