...
import type { renderVerticalSlideVideo } from './ffmpeg-video.js';
import type { Slide, VerticalVideoManifest } from './manifest.js';
import type { cropMediaImage } from './rasterizer.js';
import {
...
function createManifest(): VerticalVideoManifest {
...
function stubVerticalPresentationDependencies() {
  ...
  return {
    ...
    downscaleMedia: vi.fn<typeof downscaleMediaToWindow>(async () => {}),
  };
}
...
describe('renderSlideVideo', () => {
  it('renders source markdown for label-only links and linked licenses', async () => {
    ...
    source.licenseUrl = 'https://publisher.example.test/terms';
    ...
    const result = await renderSlideVideo({
      ...
      dependencies: {
        ...
        cropMedia: async (crop, cropPaths) => {
        ...
        renderVerticalVideo: async (videoOptions) => {
          await writeFile(videoOptions.outputPath, 'video', 'utf8');
          return { chunkEncodeMs: 0, finalEncodeMs: 0 };
        },
      },
    });
    ...
    expect(sources).toContain(
      'License: [unknown](https://publisher.example.test/terms)',
    );
  });
  ...
  it('uses manifest audio by default and removes its work directory on failure', async () => {
    ...
    const cropMedia = vi.fn(
      async (
        ...
        isolatedWorkDirectory = dirname(cropPaths.input);
        await mkdir(dirname(cropPaths.output), { recursive: true });
        await writeFile(cropPaths.output, `crop:${crop.imagePath}`, 'utf8');
      },
    );
  ...
  it('removes its work directory when media cropping fails before encoding', async () => {
    const paths = await makeRenderPaths();
    let isolatedWorkDirectory = '';
    const cropMedia = vi.fn(
      async (
        _crop: Parameters<typeof cropMediaImage>[0],
        cropPaths: Parameters<typeof cropMediaImage>[1],
      ) => {
        isolatedWorkDirectory = dirname(cropPaths.input);
        throw new Error('Sharp rejected an oversized crop');
      },
    );
    ...
    ).rejects.toThrow('Sharp rejected an oversized crop');
    await expect(access(isolatedWorkDirectory)).rejects.toMatchObject({
      code: 'ENOENT',
    });
  });
