...
describe('Rednote account snapshot login overlay fence', () => {
  it('rejects login content that replaces the page after the follower label appears', async () => {
    const body = {
      waitFor: vi.fn().mockResolvedValue(undefined),
      innerText: vi.fn().mockResolvedValue('请登录后查看完整数据\n扫码登录'),
    };
    const follower = {
      waitFor: vi.fn().mockResolvedValue(undefined),
    };
    ...
    expect(body.innerText).toHaveBeenCalledTimes(1);
