...
create table if not exists from_fed_to_chain.social_post_metrics (
  ...
  measurement_window text
    constraint social_post_metrics_measurement_window_check
    check (
...
create table if not exists from_fed_to_chain.social_daemon_state (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint social_daemon_state_id_not_blank check (
...
create table if not exists from_fed_to_chain.social_strategy_versions (
  ...
  created_at timestamptz not null default now(),
  constraint social_strategy_versions_config_is_object check (
...
drop policy if exists "Service role can manage social daemon state"
  on from_fed_to_chain.social_daemon_state;
create policy "Service role can manage social daemon state"
  on from_fed_to_chain.social_daemon_state for all to service_role
  using (true) with check (true);

drop policy if exists "Service role can manage social strategy versions"
  on from_fed_to_chain.social_strategy_versions;
create policy "Service role can manage social strategy versions"
  on from_fed_to_chain.social_strategy_versions for all to service_role
  using (true) with check (true);

drop policy if exists "Service role can manage social publish jobs"
  on from_fed_to_chain.social_publish_jobs;
create policy "Service role can manage social publish jobs"
...
revoke all on from_fed_to_chain.social_daemon_state
  from public, anon, authenticated;
revoke all on from_fed_to_chain.social_strategy_versions
  from public, anon, authenticated;
revoke all on from_fed_to_chain.social_publish_jobs
  from public, anon, authenticated;
...
-- Cost observability ledger. Runtime ownership lives in apps/control-center;
-- this file remains the repository's canonical Supabase schema snapshot.
...
values
  ('debank', 'api_unit', 'unit', 0.0002, '2026-08-01T00:00:00Z', '$200 per 1,000,000 DeBank API units'),
  ('supabase', 'pro_plan', 'month', 25, '2026-08-01T00:00:00Z', 'Supabase Pro monthly plan')
