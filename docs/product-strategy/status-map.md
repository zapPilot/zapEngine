...
Last updated: 2026-07-07 (synced with ADR 0002 A1/A2/A3 landings; A5 in progress)
...
| Append-only ledger | Partial | Event tables landed (migration `20260707000001`): `ledger_{signal,decision,plan,execution}_events` with REVOKE UPDATE/DELETE/TRUNCATE plus a guard trigger, and an insert-only `LedgerService` in account-engine (ADR 0002 A3 checked). | No producers write to the ledger yet — event wiring is deferred to 0001-M3 — and there is no API surface, projection, or frontend consumption. | Wire signal/decision/plan/execution producers (0001-M3), then fold a decision-log digest into the track-record chain (A4). |
...
| Execution persistence | Partial | `ledger_execution_events` exists and is explicitly designated the resumable per-leg state store (ADR 0002 A6 / ADR 0001 L3 journal). Deposit wizard tracks bridge and Hyperliquid arrival in client state. | Nothing writes per-leg events yet, so refresh/session loss can still strand the UX. | Wire plan ID + per-leg execution events through the ledger (A6, after D5 producer wiring). |
...
4. The append-only ledger tables now exist, but no producers write to them and nothing is exposed to the API or frontend; until wiring lands (0001-M3), public track record and resumable execution remain partial.
