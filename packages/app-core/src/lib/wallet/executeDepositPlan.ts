...
async function submitPreparedTransactionsInternal({
  ...
  result: WalletAtomicBatchResult;
...
export async function executeDepositPlan({
  ...
  // Reliable on-chain pre-flight via eth_getCode. We deliberately do NOT
  // pre-gate on wallet_getCapabilities: it is unreliable through wallet
  // abstractions (wallets omit chains rather than reporting them unsupported),
  // so a hard capability check would wrongly block the Ambire/OKX wallets.
  const { result, walletAddress, delegation } =
    await submitPreparedTransactionsInternal({
      transactions,
      walletClient,
      chainId,
    });
