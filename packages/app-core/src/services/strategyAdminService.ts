 ...
 * @example
 * ```typescript
 * const { config } = await getStrategyAdminConfig("dma_fgi_portfolio_rules_default");
 * ```
 */
 ...
 * @example
 * ```typescript
 * const { config } = await setDefaultStrategyConfig("portfolio_rules_custom_aggressive");
 * ```
 */
