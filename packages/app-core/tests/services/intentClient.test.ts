...
describe('intentClient', () => {
  it('returns configured public clients and rejects unsupported chains', () => {
    expect(getPublicClient(1)).toBeTruthy();
    expect(getPublicClient(42161)).toBeTruthy();
    expect(getPublicClient(8453)).toBeTruthy();
    expect(() => getPublicClient(10)).toThrow(
      'No public client configured for chain 10',
    );
  });
  ...
  describe('waitForBridgeCompletion', () => {
    ...
    it('omits a missing substatus from bridge failures', async () => {
      fetchMock.mockResolvedValueOnce(statusResponse({ status: 'INVALID' }));
