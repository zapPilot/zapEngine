...
function colorToDart(value) {
    ...
    const rgba = value.match(/^rgba\(\s*(\d+),\s*(\d+),\s*(\d+),\s*([0-9.]+)\s*\)$/);
    if (rgba) {
...
export function renderDartTokens(tokens) {
    const colorEntries = Object.entries(tokens.color).filter(([, value]) => typeof value === 'string');
    const colorLines = colorEntries.map(([name, value]) => `  static const ${dartName(name)} = ${colorToDart(value)};`);
...
export function writeDartTokens() {
...
//# sourceMappingURL=flutter-codegen.js.map