...
describe('LiFiBridgeAdapter', () => {
  it('only handles routes outside canonical Base/Arbitrum USDC', () => {
  ...
  it('normalizes a LI.FI quote with an approval', async () => {
    const getQuote = vi.fn().mockResolvedValue(quoteResult());
    ...
    expect(quote.calls).toEqual([quoteResult().transaction]);
  });

  it('supports quotes without approvals or route metadata', async () => {
    const rawQuote = quoteResult({ approval: undefined, route: undefined });
  ...
  it('maps terminal failures using explicit chain ids', async () => {
    ...
    ).resolves.toMatchObject({ status: 'failed', sourceTxHash: SOURCE_HASH });
  });
