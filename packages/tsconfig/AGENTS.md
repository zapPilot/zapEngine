...
- `react.json` deliberately uses `module: ESNext` + `moduleResolution: Bundler` so Vite/Next consumers don't need `.js` import extensions. Switching either to `NodeNext` will break app / landing-page imports.
